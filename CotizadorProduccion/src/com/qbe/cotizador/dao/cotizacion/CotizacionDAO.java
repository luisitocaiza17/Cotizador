...
//	}
	...
	public List<Cotizacion> buscarPorTipoObjetoParaCanal(String fecha1, String fecha2,TipoObjeto tipoObjeto,String cotizacionId,String puntoVenta,String agenteId,String identificacion,String usuarioSession, String EstadoFiltro){   
		...
		else if(EstadoFiltro.equals("CSAI"))
		{
			estado = estadoDAO.buscarPorNombreClase("Pendiente Asignar Inspector","Cotizacion");
			estado2 = estadoDAO2.buscarPorNombreClase("Pendiente Asignar Inspector","Cotizacion");
		}
		else if(EstadoFiltro.equals("CEMI"))
		{
			estado = estadoDAO.buscarPorNombreClase("Emitido","Cotizacion");
			estado2 = estadoDAO2.buscarPorNombreClase("Emitido","Cotizacion");
		}
		else if(EstadoFiltro.equals("CINS"))
		{
		...
		{
			estado = estadoDAO.buscarPorNombreClase("Pendiente","Cotizacion");
			estado2 = estadoDAO2.buscarPorNombreClase("Autorizada","Cotizacion");
		}
		...
		else if(EstadoFiltro.equals("CSAUTO"))
		{
			estado = estadoDAO.buscarPorNombreClase("Pendiente de Autorizar","Cotizacion");
			estado2 = estadoDAO2.buscarPorNombreClase("Pendiente de Autorizar","Cotizacion");
		}
		else if(EstadoFiltro.equals("AGRI"))
		{
			estado = estadoDAO.buscarPorNombreClase("Pendiente Aprobar","Cotizacion");
			estado2 = estadoDAO2.buscarPorNombreClase("Pendiente Aprobar","Cotizacion");
		}
	...
	public List<Cotizacion> buscarPorTipoObjetoxFechaPuntoVentaYEstado(String fecha1, String fecha2,TipoObjeto tipoObjeto, PuntoVenta puntoVenta,String cotizacionId,String agenteId,String identificacion,String usuarioSession, String FiltroEstado){   
		...
		else if(FiltroEstado.equals("SPINS"))
		{
			estado = estadoDAO.buscarPorNombreClase("Pendiente de Inspeccion","Cotizacion");
			estado2 = estadoDAO2.buscarPorNombreClase("Pendiente de Inspeccion","Cotizacion");
		}
		else if(FiltroEstado.equals("CEMI"))
		{
			estado = estadoDAO.buscarPorNombreClase("Emitido","Cotizacion");
			estado2 = estadoDAO2.buscarPorNombreClase("Emitido","Cotizacion");
		}
		else if(FiltroEstado.equals("SPE"))
		{
			estado = estadoDAO.buscarPorNombreClase("Pendiente","Cotizacion");
			estado2 = estadoDAO2.buscarPorNombreClase("Pendiente","Cotizacion");
		}
		...
		else if(FiltroEstado.equals("AGRI"))
		{
			estado = estadoDAO.buscarPorNombreClase("Pendiente Aprobar","Cotizacion");
			estado2 = estadoDAO2.buscarPorNombreClase("Pendiente Aprobar","Cotizacion");
		}
	...
	public Cotizacion buscarPorFactura(String numeroFactura){		
		Cotizacion cotizacion = new Cotizacion();
		List<Cotizacion> query = getEntityManager().createNamedQuery("Cotizacion.buscarPorFactura").setParameter("numeroFactura", numeroFactura).getResultList();
		if(!query.isEmpty())
			cotizacion =  query.get(0);
		return cotizacion;
	}
	
	public Cotizacion buscarPorNumeroTramite(String numeroTramite){		
		Cotizacion cotizacion = new Cotizacion();
		List<Cotizacion> query = getEntityManager().createNamedQuery("Cotizacion.buscarPorNumeroTramite").setParameter("numeroTramite", numeroTramite).getResultList();
		if(!query.isEmpty())
			cotizacion =  query.get(0);
		return cotizacion;
	}
	
	public String buscarMaxIdTramites(String numeroTramite){		
		String id="";
		List<String> query = getEntityManager().createNamedQuery("Cotizacion.buscarMaxIdTramites").setParameter("numeroTramite", numeroTramite).getResultList();
		if(!query.isEmpty())
			id =  query.get(0);
		return id;
	}
	
	public List<String> buscarRepetidos(TipoObjeto tipoObjeto,PuntoVenta puntoVenta){		
		List<String> query = getEntityManager().createNamedQuery("Cotizacion.buscarRepetidos").setParameter("puntoVenta", puntoVenta).setParameter("tipoObjeto", tipoObjeto).getResultList();
		return query;
	}
	 ...
	 * @throws Exception
	 */	
	public double[] buscarIvaSegunPuntoVenta(Cotizacion cotizacion){		
		double valorIVA = 0.0;
		double puntoVenta = 0.0;
		double respuesta [] = {0,0}; 
		try {
			boolean aplicaIVA12 = cotizacion.getPuntoVenta().getAplicaIVA12();
			DateFormat formatter;	    
			formatter = new SimpleDateFormat("dd/MM/yyyy");	       
		    java.util.Date date;
			date = formatter.parse("01/06/2016");
			java.sql.Timestamp fechaInicioIVA = new Timestamp(date.getTime());	   
		    			
		    if(cotizacion.getFechaElaboracion().before(fechaInicioIVA)){
		    	valorIVA = 12.0;
		    	puntoVenta = 1;
			}else{
				if(aplicaIVA12){
		...
		respuesta[0] = valorIVA;
		respuesta[1] = puntoVenta;
		return respuesta;
	}
