...
public class CotizacionAprobacionDAO extends EntityManagerFactoryDAO<AgriCotizacionAprobacion>{
	...
	public AgriCotizacionAprobacion buscarPorTramite(String tramite){		
		AgriCotizacionAprobacion cotizacion = new AgriCotizacionAprobacion();
		List<AgriCotizacionAprobacion> query = getEntityManager().createNamedQuery("AgriCotizacionAprobacion.buscarTramite").setParameter("NumeroTramite", tramite).getResultList();
		if(!query.isEmpty())
			cotizacion =  query.get(0);
		return cotizacion;
	}
	
	public AgriCotizacionAprobacion buscarPorTramiteCoincidencia(String tramite){		
		AgriCotizacionAprobacion cotizacion = new AgriCotizacionAprobacion();
		List<AgriCotizacionAprobacion> query = getEntityManager().createNamedQuery("AgriCotizacionAprobacion.buscarTramiteCoincidencia").setParameter("NumeroTramite", "%" +tramite+ "%").getResultList();
		if(!query.isEmpty())
			cotizacion =  query.get(0);
		return cotizacion;
	}
	...
	public List<AgriCotizacionAprobacion> consultarPorEstado2(String fInicio,
			String fFinal, String cotizacionId,
			String nroTramite, Estado estadoCotizacion){ 
	...
	public List<AgriCotizacionAprobacion> consultarPorEstado(String fInicio,
		...
		if(movimientos)
			query.setParameter("Movimiento", "SI");
		if(canalId.length()>0)
			query.setParameter("CanalId", new BigInteger(canalId));
		if(puntoVentaId.length()>0){
	...
	public List<AgriCotizacionAprobacion> buscarCotizacionesMasivas(List<String> IdsCotizacion, List<String> tramites){ 
		...
		// Agregamos la parte del where las opciones de busqueda
		if(IdsCotizacion.size()!=0){
			valoresWhereQuery += " AND c.id IN :IdsCotizacion";
		}
		if(tramites.size()!=0){
			valoresWhereQuery += " AND c.NumeroTramite IN :tramites";
		}
		...
		if(IdsCotizacion.size()!=0){
			query.setParameter("IdsCotizacion",IdsCotizacion);
		}
		if(tramites.size()!=0){
			query.setParameter("tramites", tramites);
		}
	...
	public long buscarCotizacionesMasivasNum(List<String> IdsCotizacion, List<String> tramites){ 
