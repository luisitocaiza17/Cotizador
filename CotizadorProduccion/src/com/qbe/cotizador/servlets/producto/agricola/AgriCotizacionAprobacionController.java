			...
			if (tipoConsulta.equals("Aprobacion")) {
				...
				if (AprobacionStatus.equals("AP")) {
					...
					if (cotizacionC.getEstado().getId().equals(estadoAprobar.getId()))
						mensajeEstado = mensajeEstado+ "La cotización ya fue aprobada anteriormente. ";
					...
					if (!cotizacionC.getEstado().getId().equals(estadoAprobar.getId())){
						...
						mensajeEstado = mensajeEstado+"La cotización esta en estado "+estadoAprobar.getNombre();
					}
					...
					if (cotizacionC.getSumaAseguradaTotal()==0)
						mensajeEstado = mensajeEstado+"La cotización no tiene valor Suma Asegurada. ";
					if (cotizacionC.getPrimaNetaTotal().equals("0"))
						mensajeEstado = mensajeEstado+"La cotización no tiene valor Prima. ";
					if (cotizacionC.getTotalFactura()==0)
						mensajeEstado = mensajeEstado+"La cotización no tiene valor Total de Factura. ";
						...
						if (canalNotificacion.getFormaNotificacion() == 2) {
							if (cotizacionC.getNumeroTramite() != null) {
								...
								if (resultado != null) {
									...
									result.put("mensajeEstado","No se ha podido aprobar la cotización. "+ms+" Por favor vuelva a intentar más tarde.");
								}
								//TODO: Cargar el log de respuesta de sucre para probacion o rechazo
								...
								result.put("mensajeEstado","No se ha podido aprobar. La cotización no tiene número de tramite. ");
							}
					} else {
						// Actualiza tabla detalle cotizacion si es un recalculo
						if (IsRecalculo.equals("true"))
					...
					if (cotizacionC.getEstado().getId().equals(estadoRechazado.getId()))
						MensajeRechazo = "La cotización ya fue rechazada anteriormente";
							...
							if (canalNotificacion.getFormaNotificacion() == 2) {
								if (cotizacionC.getNumeroTramite() != null) {
									...
									if (resultado != null) {
										...
										String ms = "No se obtuvo respuesta de confirmación de Sucre.";
										DatosRecibidos = DatosRecibidos+ ms;
										result.put("mensajeEstado","No se ha podido rechazar la cotización. "+ms+" Por favor vuelva a intentar más tarde.");
									}
									//TODO: Cargar el log de respuesta de sucre para probacion o rechazo
									auditoria.setTramite(cotizacionC.getNumeroTramite());
									auditoria.setObjeto(DatosRecibidos);
									...
									result.put("mensajeEstado","No se ha podido rechazar. La cotización no tiene número de tramite. ");
								}
						} else {
...
//				result.put("tipo_seguro", cotAct.getTipoSeguro());
			...
			if (tipoConsulta.equals("importar")) {
				...
				if(usuarioSession==null)
					throw new Exception("El usuario no esta en seccion, debe refrescar la página");
			...
			for (Cotizacion nuevaCotizacion : listado) {//recorremos el listado de cotizaciones que viende desde excel				
				...
				try {
					...
					if (cotizacionC.getId() != null) {//verificamos si esta registrado en base
						...
						if (cotizacionC.getEstado().getId().equals(estadoRechazado.getId()))
							mensaje = mensaje + " la cotización ya fue rechazada anteriormente,";
						...
						if (mensaje.equals("")) {//Si todo esta correcto
								...
								if (cotizacionC.getNumeroTramite() != null) {//si tiene un numero de tramite
									...
									if (resultado != null) {
										...
										if (resultado.getRespuesta().getObservaciones().toUpperCase().equals("OK")) {
											...
											mensaje = "La Cotización ha sido "+proceso+" correctamente";
										} else {
									...
									mensaje = "No se ha podido "+proceso+" La cotización no tiene número de tramite. ";
								}
							}else {
								///Actualiza estado tabla cotizacion
							...
							mensaje = "La cotización no se ha podido "+proceso+": "+mensaje.substring(0,mensaje.length()-1);
						...
						mensaje = ("No se pudo "+proceso+" la cotización, la cotización no se encuentra registrada.");						
					}
				} catch (Exception e) {
				...
				try {
					...
					if (cotizacionC.getId() != null) {
						...
						if (cotAct.getDireccionSiembra()==null || cotAct.getDireccionSiembra().equals(""))
							mensaje = mensaje + " la cotizacion no tiene  dirección,";
						...
						if (mensaje.equals("")) {
							...
							if (resultado.isEmitidoCorrectamente()) {
								...
								mensaje = ("La cotización no se ha podido emitir. " + resultado.getMensaje());
								estado="Error";
							}
						} else{
							mensaje = "La cotización no se ha podido emitir: "+ mensaje.substring(0, mensaje.length() - 1);
							estado="Error";
						}
						detalleProceso.put("cotizacion", cotizacionC.getId());
						detalleProceso.put("detalle", mensaje);
						...
						mensaje = ("No se pudo emitir la cotización, la cotización no se encuentra registrada.");
						estado="Error";
						throw new Exception();
					}
				} catch (Exception e) {
					mensaje = "No se pudo emitir la cotización, se ha producido un error. "+e.getMessage();
					estado="Error";
					detalleProceso.put("cotizacion", nuevaCotizacion.getId());
					detalleProceso.put("detalle", mensaje);
					detalleProceso.put("estado", estado);
			...
			for (Cotizacion nuevaCotizacion : listado) {
				...
				try {
					...
					if (cotizacionC.getId() != null) {
						...
						mensaje = ("No se pudo revocar la cotización, la cotización no se encuentra registrada.");
						estado="Error";
						throw new Exception();
					}
				} catch (Exception e) {
					mensaje = "No se pudo revocar la cotización, se ha producido un error. "+e.getMessage();
					estado="Error";
					detalleProceso.put("cotizacion", nuevaCotizacion.getId());
					detalleProceso.put("detalle", mensaje);
					detalleProceso.put("estado", estado);
	...
	private void EnviarMailCambioEstado(Cotizacion cotizacion,
		...
		if (status.equals("AP")) {
			estado = "aprobadas";
			detalle = "A continuación se adjunta el documento de pre-poliza.";
		} else {
			estado = "rechazadas";
			detalle = "";
		}
