...
public class AgriProcesarCotizacionOffline {
	public static AgriRespuestaOffline procesarCotizacion(CotizacionAgricola nuevaCotizacion, String tipoOrigen,String correo,String CCCorreos){
		...
		try{
			...
			VariableSistema variableSistema=variableSistemaDAO.buscarPorNombre("FECHA_VERSION_OFFLINE");
			
			
			Date fechaActual = new Date();
			SimpleDateFormat dt = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
			Date fechaCambioVersion = dt.parse(variableSistema.getValor());
			...
			if(fechaActual.after(fechaCambioVersion))	{
				if(nuevaCotizacion.getTipoSeguro()!=2){//version actual del cotizador
					throw new Exception("La cotización fue realizada desde una versión anterior del cotizador, por favor realícela desde la versión actual: 2.0");
				}
			}			
			...
			if(fechaEmision.after(fechaAplicaIva))	{
				//verificacion de uso de version 2.0 de cotizador offline, verificamos que el iva sea 12%
				variableSistema=variableSistemaDAO.buscarPorNombre("IVA");
				//redondeo para verificacion
				BigDecimal T = new BigDecimal(""+nuevaCotizacion.getIva());
				BigDecimal roundOffT = T.setScale(2, BigDecimal.ROUND_HALF_UP);
				double valor=Double.parseDouble(""+roundOffT);
				
				double valorIvaCotizacion= valor;
							
				//recalculo de iva para verificacion de coincidencias
				
				double valorPrima=nuevaCotizacion.getPrimaNeta();
				double segBancos=nuevaCotizacion.getSuperBancos();
				double segCampesinos=nuevaCotizacion.getSeguroCampesino();
				double segDerecho=nuevaCotizacion.getDerechoEmision();
				
				double porsentajeIva=Double.parseDouble(variableSistema.getValor());
				
				double valorIvaRecalculado=((valorPrima+segBancos+segCampesinos+segDerecho)*porsentajeIva)/100;
				
				T = new BigDecimal(""+valorIvaRecalculado);
				roundOffT = T.setScale(2, BigDecimal.ROUND_HALF_UP);
				valor=Double.parseDouble(""+roundOffT);
				valorIvaRecalculado=valor;
				
				double totalFactura=valorPrima+segBancos+segCampesinos+segDerecho+valorIvaRecalculado;
				
				T = new BigDecimal(""+totalFactura);
				roundOffT = T.setScale(2, BigDecimal.ROUND_HALF_UP);
				valor=Double.parseDouble(""+roundOffT);
				totalFactura=valor;			
				
				nuevaCotizacion.setIva(valorIvaRecalculado);
				nuevaCotizacion.setTotalPrima(totalFactura);
			...
			String reglaId=""+nuevaCotizacion.getReglaId();
			if (!reglaId.equalsIgnoreCase("null"))
				agriObjetoCotizacion.setTipoCalculo(reglaId);
			//TODO: Grabar la agencia del offline e el campo entidad financiera del objeto 
			String agenciaId=""+nuevaCotizacion.getAgencia();
			if (!agenciaId.equalsIgnoreCase("null"))
				agriObjetoCotizacion.setCodEntidadFinanciera(agenciaId);
