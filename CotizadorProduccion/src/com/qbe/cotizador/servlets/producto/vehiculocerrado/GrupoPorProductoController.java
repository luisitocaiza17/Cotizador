...
public class GrupoPorProductoController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			// Encontramos los diferentes grupos por producto (Todos los productos)
			if(tipoConsulta.equals("encontrarTodosPorGrupo") && !tipoObjeto.equals("No Tiene")){
				...
				TipoGrupo tipoGrupo = tipoProductoDAO.buscarPorNombre(tipoObjeto);
				GrupoProducto grupoProducto =  grupoProductoDAO.buscarPorId(grupoProductoId);							
				List<GrupoPorProducto> gruposPorProductoListado = new ArrayList<GrupoPorProducto>();
				...
				if(usuarioXPuntoVenta.getId()!=null){
					...
					if(puntoVenta.getEsMatriz()){
						...
						for(PuntoVenta puntoVentaDeMatriz:puntosVentaPoseeMatriz){
							productosXPuntoVentas.addAll((List<ProductoXPuntoVenta>) productoXPuntoVentaDAO.buscarPorGrupoPunto(puntoVentaDeMatriz,tipoGrupo));
						}
						
						gruposPorProductoListado = grupoPorProductoDAO.buscarTodosPorGrupoTipoGrupo(grupoProducto, tipoGrupo);
						
						for(int i=0; i<gruposPorProductoListado.size(); i++){
							for(int j=0; j<productosXPuntoVentas.size(); j++){
							
							grupoPorProducto = gruposPorProductoListado.get(i);
			...
			// Encontramos los diferentes grupos producto
			if(tipoConsulta.equals("encontrarTodosGrupoProducto") && !tipoObjeto.equals("No tiene")){
				Usuario usuario=new Usuario();
				if(request.getSession().getAttribute("usuario")!=null)
					usuario = (Usuario)request.getSession().getAttribute("usuario");
				...
				TipoGrupoDAO tipoProductoDAO = new TipoGrupoDAO();
				TipoGrupo tipoGrupo = tipoProductoDAO.buscarPorNombre(tipoObjeto);								
				List<GrupoPorProducto> results = grupoPorProductoDAO.buscarActivosPorTipoGrupo(tipoGrupo);
				...
				List<ProductoXPuntoVenta> pxpvs = new ArrayList<ProductoXPuntoVenta>();
									
				for(PuntoVenta puntoVentaDeMatriz:puntosVentaPoseeMatriz){
					pxpvs.addAll((List<ProductoXPuntoVenta>) pxpvDAO.buscarActivosPorGrupoPunto(puntoVentaDeMatriz,tipoGrupo));
				}
