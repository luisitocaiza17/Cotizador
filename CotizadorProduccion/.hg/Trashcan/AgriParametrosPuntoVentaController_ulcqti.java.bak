...
public class AgriParametrosPuntoVentaController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(!agenteId.equals(""))
				parametroPymeXPV.setAgenteId(agenteId);
			
			if(!comision.equals(""))
				parametroPymeXPV.setPorcentajeComision(comision);
			...
			if(tipoConsulta.equals("buscarAgente")){
				AgenteDAO agenteDAO = new AgenteDAO();
				List<Agente> agente= agenteDAO.buscarTodos();
				JSONObject AgenteObject = new JSONObject();
				JSONArray AgenteArray = new JSONArray();
				for(Agente rs :agente){
					AgenteObject.put("nombre", rs.getEntidad().getNombreCompleto()+" (Ensurance Id: "+rs.getAgeEnsurance()+")");
					AgenteObject.put("id",rs.getId());
					AgenteArray.add(AgenteObject);
				}
				result.put("listaAgentes", AgenteArray);
			}
			...
			if(tipoConsulta.equals("buscarPorId")){
				...
				result.put("agenteId", parametroPymeXPV.getAgenteId());
				result.put("comision", parametroPymeXPV.getPorcentajeComision());
