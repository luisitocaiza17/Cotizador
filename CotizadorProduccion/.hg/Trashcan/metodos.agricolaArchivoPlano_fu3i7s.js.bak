...
function mostrarTipoIdentificacionAsegurado() {
	...
	if (tipoIdentificacionId == '' || tipoIdentificacionId == '1'
		...
		$("#ubicacionPersonaNatural").css("visibility","visible");
		$("#ubicacionPersonaNatural").show();
		$("#datosPoliticaNatural").css("visibility","visible");
		$("#datosPoliticaNatural").show();
		$("#situacionFinancieraNatural").css("visibility","visible");
		$("#situacionFinancieraNatural").show();
		$("#ubicacionPersonaJuridica").css("visibility","hidden");
		$("#ubicacionPersonaJuridica").hide();
		$("#datosPoliticaJuridica").css("visibility","hidden");
		$("#datosPoliticaJuridica").hide();
		$("#situacionFinancieraJuridica").css("visibility","hidden");
		$("#situacionFinancieraJuridica").hide();
	} else {
		$("#nombres_asegurado").val("");
		$("#apellidos_asegurado").val("");
		$("#ubicacionPersonaNatural").css("visibility","hidden");
		$("#ubicacionPersonaNatural").hide();
		$("#datosPoliticaNatural").css("visibility","hidden");
		$("#datosPoliticaNatural").hide();
		$("#situacionFinancieraNatural").css("visibility","hidden");
		$("#situacionFinancieraNatural").hide();
		$("#ubicacionPersonaJuridica").css("visibility","visible");
		$("#ubicacionPersonaJuridica").show();
		$("#datosPoliticaJuridica").css("visibility","visible");
		$("#datosPoliticaJuridica").show();
		$("#situacionFinancieraJuridica").css("visibility","visible");
	...
	$
			.ajax({
				...
				success : function(data) {
					...
					for (var j = 0; j < sucursales.length; j++) {
						...
						for (var i = 0; i < data.puntosVenta.length; i++) {
							if (data.puntosVenta[i].sucursal == sucursales[j].id) {
								...
								if(data.puntosVenta[i].nombre=='PRONACA'){
					...
					obtenerSucursalesXCanal($("#punto_venta").val());
				}
			});
}
...
function obtenerProductosPorGrupoProducto(producto) {
	...
	$
			...
			obtenerPuntosVentaPorProducto('', $("#productos option:selected").val());
...
function cambiarEstadoAgricola(estado) {
	$("#msgPopupEmitidoRechazado").hide();
	$("#msgPopupFinalizadoCorrectamente").hide();
	$.ajax({
...
function emitirPolizaAgricola() {
	...
	if ($("#fecha_inicio_vigencia").val() == "") {
		...
		$.ajax({
			...
			success : function(data) {
				$("#cotizacion_id").text(data.cotizacionId);
				var mensaje = data.mensajeEmision;
				var Fact = data.NroFactura;
				var error = data.error;
				if (typeof error != "undefined"){
					$('#btnGenerarPoliza').hide();
					$('#mensaje').html("Se ha producido un error en el proceso de emision: "+error);
				}
				if (typeof Fact != "undefined"){
					$('#btnGenerarPoliza').show();
					$('#mensaje').html("El numero de Factura de la cotizacion es: "+Fact);
				}
				else if (typeof mensaje != "undefined"){
					$('#btnGenerarPoliza').hide();
					$('#mensaje').html("Advertencia: "+mensaje);
				}
				// Validacion poner en la URL el id de la cotizacion
				var valorId = getParameterByName("id");
				if (valorId != null) {
	...
	if (datos != null) {
		...
		if (formulario == "asegurado") {
			...
			if (datos.tipoIdentificacion == "4") {
				...
				$("#filaJuridicaAsegurado").css("visibility","visible");
				$("#filaNaturalAsegurado").css("visibility","hidden");

			} else {
				$("#nombre_direccion_asegurado").val(datos.nombre + " " + datos.apellido);
				$("#filaNaturalAsegurado").css("visibility","visible");
				$("#filaJuridicaAsegurado").css("visibility","hidden");
			}
...
function cargarValoresCotizacion() {
	$.ajax({
		...
		success : function(data) {
			...
			$("#resumenTotalPago").text("Total a Pagar: "+data.valorTotal);
		}
	});
}
...
function cargarPorIdentificacion(formulario, valor) {
	...
	if ((identificacion.length >= 10 && $("#tipo_identificacion_principal").val() != 2)
			|| (identificacion.length >= 5 && ($("#tipo_identificacion_principal").val() == 2 
...
function guardarProductoAgricola() {
	...
	if(parseFloat($("#hectareas_asegurables").val()) > parseFloat($("#hectareas_lote").val())){
		$("#msgPopupFichaAgricolaError").html("El n&uacute;mero de Hectareas Lote, no puede ser menor al n&uacute;mero de Hectareas Asegurables");
	...
	$.ajax({url : "../AgriObjetoCotizacionController",
		data : {
			"cotizacionId" : $("#cotizacion_id").text(),
			"tipoConsulta" : "crearArchivoPlano",
...
function generarReporteCotizacion() {
	...
	alert("Por favor una vez descargada la poliza, presionar el boton Finalizar para que la cotización sea valida.");	
	var cotizacionId=$("#cotizacion_id").text();
	var numHectareas=$("#hectareas_asegurables").val();	
	var previaId=$("#cargaPreviaId").val();	
	var rutaTota="../AgriCotizacionReporte?proceso=Pronaca&tipoConsulta=generarReporte&codigoCotizacion=" +cotizacionId+"&numHectareas="+numHectareas+"&previaID="+previaId;
	window.open('../AgriCotizacionReporte?proceso=Pronaca&tipoConsulta=generarReporte&codigoCotizacion=' +cotizacionId+'&numHectareas='+numHectareas+'&previaID='+previaId);
	alert("Por favor precionar boton finalizar en la ventana de cotización");
}

function guardarPago(valor) {
	
	if(!$("#cboFpFormaPago").valid())
		return false;
...
function addRequiredp1(){	
	$("#ubicacion_provincia").prop( "required", true );
	$("#ubicacion_Canton").prop( "required", true );
	$("#ubicacion_Parroquia").prop( "required", true );
	$("#ubicacion_direccion").prop( "required", true );
	$("#sucursal_canal").prop( "required", true );
	$("#required").prop( "required", true );
	$("#required").prop( "required", true );
	$("#hectareas_lote").prop( "required", true );
	$("#hectareas_asegurables").prop( "required", true );
	$("#fecha_siembra").prop( "required", true );
	$("#ubicacion_altitud").prop( "required", true );
	$("#ubicacion_provincia").prop( "required", true );
	$("#ubicacion_provincia").prop( "required", true );
	$("#ubicacion_provincia").prop( "required", true );
}*/

function validatePag1(){
	var valid = true;
	if(!$("#grupo_productos").valid())
		valid = false;
	if(!$("#productos").valid())
		valid = false;
	if(!$("#canal").valid())
		valid = false;
	if(!$("#punto_venta").valid())
		valid = false;
	if(!$("#agentes").valid())
		valid = false;
	if(!$("#identificacion").valid())
		valid = false;	
	if(!$("#tipo_identificacion_principal").valid())
		valid = false;
	if($("#tipo_identificacion_principal").val()==4){
		if(!$("#es_contribuyente").valid())
			valid = false;
		if(!$("#nombre_completo").valid())
			valid = false;
	}
	if($("#tipo_identificacion_principal").val()!=4){
		if(!$("#nombres").valid())
...
function validatePag2(){
	var valid = true;
	if(!$("#ubicacion_provincia").valid())
		valid = false;
	if(!$("#ubicacion_Canton").valid())
		valid = false;
	if(!$("#ubicacion_Parroquia").valid())
		valid = false;
	if(!$("#ubicacion_direccion").valid())
		valid = false;
	//if(!$("#sucursal_canal").valid())
		//valid = false;	
	if(!$("#tipo_cultivo").valid())
			valid = false;
	//if($("#cultivoPerenne").css("visibility") == "visible"){		
		//if(!$("#tipo_poliza").valid())
			//valid = false;	
		//if(!$("#anios_cultivo").valid())
			//valid = false;
	//}
	if(!$("#hectareas_lote").valid())
		valid = false;
	if(!$("#hectareas_asegurables").valid())
		valid = false;
	if(!$("#fecha_siembra").valid())
		valid = false;
	//if(!$("#ubicacion_altitud").valid())
		//valid = false;	
...
function validatePag3(){
	var valid = true;
	if(!$("#cboFpFormaPago").valid())
		valid = false;
	if(!$("#tipo_identificacion_asegurado").valid())
		valid = false;	
	/*if(!$("#tipo_identificacion_asegurado").valid())
		valid = false;
	if(!$("#identificacion_asegurado").valid())
		valid = false;	
	if($("#filaJuridicaAsegurado").css("visibility") == "visible"){		
		if(!$("#nombre_completo_asegurado").valid())
			valid = false;	
	}else{
		if(!$("#nombres_asegurado").valid())
		valid = false;
		if(!$("#apellidos_asegurado").valid())
			valid = false;
	}*/
...
function validatePag4(){
	var valid = true;
	if($("#ubicacionPersonaNatural").css("visibility") == "visible"){
		if(!$("#zona_direccion_natural").valid())
		valid = false;
	
		if(!$("#provincia_direccion_cliente_natural").valid())
				valid = false;
		if($("#canton_cliente_select").css("visibility") == "visible"){		
			if(!$("#canton_direccion_cliente_natural").valid())
				valid = false;	
		}
		if($("#parroquia_cliente_select").css("visibility") == "visible"){		
			if(!$("#parroquia_direccion_cliente_natural").valid())
				valid = false;	
		}
		if($("#ciudad_cliente_select").css("visibility") == "visible"){		
			if(!$("#ciudad_direccion_cliente_natural").valid())
				valid = false;	
		}
			
		if(!$("#calle_principal_direccion_natural").valid())
			valid = false;	
		
		if(!$("#numero_direccion_natural").valid())
			valid = false;

		if(!$("#calle_secundaria_direccion_natural").valid())
				valid = false;

		if(!$("#referencia_direccion_natural").valid())
				valid = false;

		if(!$("#fecha_nacimiento_natural").valid())
				valid = false;

		if(!$("#telefono_cliente_natural").valid())
				valid = false;
			
		if(!$("#celular_cliente_natural").valid())
					valid = false;			
					
	}
	
	if($("#ubicacionPersonaJuridica").css("visibility") == "visible"){
		if(!$("#zona_direccion_matriz_juridica").valid())
		valid = false;
	
		if(!$("#provincia_direccion_matriz_juridica").valid())
				valid = false;
		if($("#canton_matriz_select").css("visibility") == "visible"){		
			if(!$("#canton_direccion_matriz_juridica").valid())
				valid = false;	
		}
		if($("#parroquia_matriz_select").css("visibility") == "visible"){		
			if(!$("#parroquia_direccion_matriz_juridica").valid())
				valid = false;	
		}
		if($("#ciudad_matriz_select").css("visibility") == "visible"){		
			if(!$("#ciudad_direccion_matriz_juridica").valid())
				valid = false;	
		}
			
		if(!$("#calle_principal_direccion_juridica").valid())
			valid = false;	
		
		if(!$("#numero_direccion_juridica").valid())
			valid = false;

		if(!$("#calle_secundaria_direccion_juridica").valid())
				valid = false;

		if(!$("#referencia_direccion_juridica").valid())
				valid = false;

		if(!$("#telefono_juridica").valid())
				valid = false;

		if(!$("#celular_representante_legal").valid())
				valid = false;			
			
	}
	
	if($("#datosPoliticaNatural").css("visibility") == "visible"){
			
			
	}
	if($("#datosPoliticaJuridica").css("visibility") == "visible"){
...
/*Metodos carga previa*/
function cargarTodas(){	
		$('#add').modal('show');
		$("#preCotizaciones").show("slow");
		...
		$.ajax({
			...
			success : function(data) {				
				...
				var listCotizacion = data.listCotizacion;				
				$.each(listCotizacion,function(index) {
					var montoAsegurado = 0;
					if(parseFloat(listCotizacion[index].montoAsegurado).toFixed(2) != "NaN")
						montoAsegurado = parseFloat(listCotizacion[index].montoAsegurado).toFixed(2);
					else
						montoAsegurado = "0.00";
				
						$("#dataTable tbody").append(
								"<tr class='odd gradeX'>"
										+ "<td relation='cliente'>"+ listCotizacion[index].cliente+ "</td>"
										+ "<td relation='identificacion'>"+ listCotizacion[index].identificacion+ "</td>"
										+ "<td relation='montoAsegurado'>"+ montoAsegurado+ "</td>"
										+ "<td relation='fechaSolicitud'>"+ listCotizacion[index].fechaSolicitud+ "</td>"
										+ "<td relation='fechaSiembra'>"+ listCotizacion[index].fechaSiembra+ "</td>"
										+ "<td relation='tipoCultivo'>"+ listCotizacion[index].tipoCultivo+ "</td>"
										+ "<td relation='hasAseguradas'>"+ listCotizacion[index].hasAseguradas+ "</td>"
										+ "<td relation='hasLote'>"+ listCotizacion[index].hasLote+ "</td>"														
										+ "<td relation='provincia'>"+ listCotizacion[index].provincia+ "</td>"
										+ "<td relation='canton'>"+ listCotizacion[index].canton+ "</td>"																										
										+ "<td width='80px' align='center'>"
											+ " <button type='button'  name='actualizar-btn' class='btn btn-success btn-xs actualizar-btn' onclick='cargarCotizacion("
											+ listCotizacion[index].id
											+ ")'>"
											+ " <span class='glyphicon glyphicon glyphicon-edit'></span> Generar"
											+ " </button>"
											+ " <button type='button' class='btn btn-danger btn-xs eliminar-btn' onclick='eliminarCotizacion("
											+ listCotizacion[index].id
											+ ");'>"
			...
			$.ajax({
				...
				success : function(data) {
					...
					if(data.identificacion.length==10){
						$("#tipo_identificacion_principal").val(1);
					}else{
						if(data.identificacion.length==13){
							$("#tipo_identificacion_principal").val(3);
						}else{
							$("#tipo_identificacion_principal").val(4);
						}
					}
					$("#cargaPreviaId").val(arg);
					if(typeof $("#nombres").val() == "undefined" || $("#nombres").val()=="")
						$("#nombres").val(data.cliente);
					if(typeof $("#apellidos").val() == "undefined" || $("#apellidos").val()=="")
						$("#apellidos").val(data.apellidos);
					$("#celular").val(data.telefono);
					$("#ubicacion_direccion").val(data.ubicacion);
					$("#ubicacion_latitud").val(data.gpsX);
					$("#ubicacion_longitud").val(data.gpsY);
					$("#monto_credito").val(data.montoAsegurado);
					$("#hectareas_lote").val(data.hasLote);
					...
					$("#fecha_credito").val(data.fechaSolicitud);
					$("#fecha_siembra").val(data.fechaSiembra);					
					$("#ubicacion_altitud").removeAttr('required');
					$("#email").removeAttr('required');			
				}
			});		
			
			$("#preCotizaciones").hide("slow");
	}
