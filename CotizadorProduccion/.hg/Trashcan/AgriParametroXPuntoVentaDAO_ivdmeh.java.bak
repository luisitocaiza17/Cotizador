...
public class AgriParametroXPuntoVentaDAO extends EntityManagerFactoryDAO<AgriParametroXPuntoVenta> {
	...
	public AgriParametroXPuntoVenta buscarCodigoIntegracion(String  codigoIntegracion)
	{
		AgriParametroXPuntoVenta agriParametro = new AgriParametroXPuntoVenta();
		List<AgriParametroXPuntoVenta> result = getEntityManager().createNamedQuery("AgriParametroXPuntoVenta.obtenerCodigoIntegracion").setParameter("codigoIntegracion", codigoIntegracion).getResultList();
		if (result.size()>0)
			agriParametro=result.get(0);
		return agriParametro;
	}
