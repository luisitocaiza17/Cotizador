...
<head>
	...
	<script>
		...
		$(document).ready(function() {
			activarMenu("CotizacionesPendientesPymes");		 	
			 $('#dataTable').hide();
				...
				if(fechaInicio != "" && fechaFin=="" ){
	    		 ...
	    		 $.ajax({
	 				...
	 				data : {
	 					...
	 					"tipoConsulta" : "encontrarAprobadoPorCanalPymes",
	 					...
	 					if(listadoCotizacion.length > 0){
	 						$.each(listadoCotizacion, function(index){
	 							...
	 							" <td width='80px' align='center'>" +
	 								...
	 								" <input type='hidden' id='tipo_"+listadoCotizacion[index].codigo+"'  value='"+ listadoCotizacion[index].tipo_producto +"'/>" +
	 								" <button type='button' class='btn btn-success btn-xs actualizar-btn'>" +
	 								"<span class='glyphicon glyphicon glyphicon-remove' id='delete-record'></span>&nbsp; Autorizar para emisión &nbsp;" +
	 								" </button>" +
	 							"</td>" +
	 						"</tr>";
	 								auxiliar +=	aux;
	 						...
	 						$('#dataTable').dataTable( {	 							
        						...
        						"fnDrawCallback": function( oSettings ) {
        							...
        							$(".actualizar-btn").bind({click: function(event) {
	        								var r = confirm("Esta seguro que desea autorizar la cotización seleccionada.?");
	    									if (r == true){
		        								var id=$(this).parent().parent().children().first().text();
		        								autorizarCotizacion(id);				
	        								}
        								}
        								});
        						    }
	    		 ...
	    		 function autorizarCotizacion(id){
	    			 $.ajax({
							...
							data : {
								"cotizacionId" : id,
								"tipoConsulta" : "autorizarCotizacion",
								"estadoConsulta" : estadoConsulta
							},
							type : 'POST',
							datatype : 'json',
							success : function(data) {
								if(data.success)
									alert("La cotizacion ha sido autorizada correctamente, ya puede emitir.");
								else
									alert(data.error);
							}
						});
