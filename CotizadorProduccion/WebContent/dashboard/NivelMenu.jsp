...
<head>
	...
	<title>Nivel Menu - CotizadorQBE</title>
	...
	<script>
		...
		$(document).ready(function() {
			activarMenu("NivelMenu");
			
			$.ajax({
				url : '../NivelMenuController',
				data : {
				...
				success : function(data) {
					...
					if(data.numRegistros > 0){
						var listadoNivelMenu = data.listadoNivelMenu;
						$.each(listadoNivelMenu, function(index){
							$("#dataTableContent").append("	<tr class='odd gradeX'>" +
									" <td relation='nombre'>"+ listadoNivelMenu[index].nombre +"</td>" +
									" <td width='175px'>" +
										" <input type='hidden' value='"+ listadoNivelMenu[index].codigo +"'/>" +
						...
						$(".eliminar-btn").bind({click: function() {
								var r = confirm("Seguro que desea eliminar el NivelMenu " + $(this).parent().parent().children().first().text());
								if (r == true){
									codigo = $(this).parent().children().first().val();
									nombre = ""; matriz = "";  codigoEnsurance="";
				...
				<div class="modal-content">
					<form id="formCrud">
						<div class="modal-header">
							...
							<h4 class="modal-title" id="myModalLabel">Nuevo Nivel Menu</h4>
						</div>
						<div class="modal-body">
							<div class="alert alert-success" id="msgPopup">El Nivel Menu se grabo con exito.</div>
