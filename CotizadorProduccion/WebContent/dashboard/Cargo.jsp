...
<head>
	...
	<title>Cargo - CotizadorQBE</title>
	...
	<script>
		...
		$(document).ready(function() {
					...
					if(data.numRegistros > 0){
						...
						$.each(listadoCargo, function(index){
							$("#dataTableContent").append("	<tr class='odd gradeX'>" +
									" <td relation='nombre'>"+ listadoCargo[index].nombre +"</td>" +
									" <td relation='activo'>"+ listadoCargo[index].activo +"</td>" +
									" <td width='175px'>" +
										" <input type='hidden' value='"+ listadoCargo[index].codigo +"'/>" +
						...
						$(".eliminar-btn").bind({click: function() {
								var r = confirm("Seguro que desea eliminar el Cargo " + $(this).parent().parent().children().first().text());
								if (r == true){
									codigo = $(this).parent().children().first().val();
									nombre = ""; matriz = ""; activo = ""; codigoEnsurance="";
									tipoConsulta = "eliminar";
									enviarDatos(codigo, nombre, activo, tipoConsulta);
			...
			function enviarDatos(codigo, nombre, activo, tipoConsulta){
				$.ajax({
					url : '../CargoController',
					data : {
						"codigo" : codigo,
				...
				<div class="modal-content">
					<form id="formCrud">
						...
						<div class="modal-body">
							<div class="alert alert-success" id="msgPopup">El Cargo se grabo con exito.</div>
