...
<head>
	...
	<script>
		...
		$(document).ready(function() {
			activarMenu("HistorialVentasCerrados");		 	
			 $('#dataTable').hide();
	    	 ...
	    	 $('#ConsultaBtn').click(function(){
	 				...
	 				success : function(data) {					
	 					...
	 					if(listadoCotizacion.length > 0){
	 						$.each(listadoCotizacion, function(index){
	 							...
	 							" <td relation='version_tarificador' style='display:none'>"+ listadoCotizacion[index].version_tarificador +"</td>" +
	 							" <td width='80px' align='center'>" +
	 								" <input type='hidden'  value='"+ listadoCotizacion[index].codigo +"'/>" +
	 								" <button type='button' class='btn btn-success btn-xs actualizar-btn' onClick='actualizar(event)'>" +
							...
							if($(this).parent().prev().prev().prev().prev().prev().text() == "Vehiculos Livianos")
								window.open('Cotizacion.jsp?id='+id+'&v='+$(this).parent().prev().text());
														
							if($(this).parent().prev().prev().prev().prev().prev().text() == "Vehiculos Cerrados Livianos")
								window.open('CotizacionLivianos.jsp?id='+id+'&v='+$(this).parent().prev().text());							
							
							if($(this).parent().prev().prev().prev().prev().prev().text() == "Vehiculos Cerrados Motos")
								window.open('CotizacionMotos.jsp?id='+id+'&v='+$(this).parent().prev().text());
							
							if($(this).parent().prev().prev().prev().prev().prev().text() == "Vehiculos Cerrados Pesados")
								window.open('CotizacionPesados.jsp?id='+id+'&v='+$(this).parent().prev().text()); 
							
							if($(this).parent().prev().prev().prev().prev().prev().text() == "Vehiculos Cerrados Publicos")
								window.open('CotizacionPublicos.jsp?id='+id+'&v='+$(this).parent().prev().text());
							}
							});
							
							$(".eliminar-btn").bind({click: function(event) {
								var r = window.confirm("Seguro que desea eliminar la Cotizacion " + $(this).parent().parent().children().first().text());
								if (r == true){
				...
				//Metodo validacion de fechas buscador
					...
					function actualizar(event) {
						...
						if($(target).attr("type")==null||$(target).attr("type")!="button")
							target=$(target).parent();
						
						var id=$(target).parent().parent().children().first().text();
						
						if($(target).parent().prev().prev().prev().prev().prev().prev().text() == "Vehiculos Livianos")
							window.open('Cotizacion.jsp?id='+id+'&v=V'+$(target).parent().prev().text());
						
						if($(target).parent().prev().prev().prev().prev().prev().prev().text() == "Vehiculos Cerrados Livianos")
							window.open('CotizacionLivianos.jsp?id='+id+'&v=V'+$(target).parent().prev().text());							
						
						if($(target).parent().prev().prev().prev().prev().prev().prev().text() == "Vehiculos Cerrados Motos")
							window.open('CotizacionMotos.jsp?id='+id+'&v=V'+$(target).parent().prev().text());
						
						if($(target).parent().prev().prev().prev().prev().prev().prev().text() == "Vehiculos Cerrados Pesados")
							window.open('CotizacionPesados.jsp?id='+id+'&v=V'+$(target).parent().prev().text()); 
						
						if($(target).parent().prev().prev().prev().prev().prev().prev().text() == "Vehiculos Cerrados Publicos")
							window.open('CotizacionPublicos.jsp?id='+id+'&v=V'+$(target).parent().prev().text());
						}
						...
						function eliminar(event) {
							...
							var r = window.confirm("Seguro que desea eliminar la Cotizacion " + $(this).parent().parent().children().first().text());
							if (r == true){
								codigo = $(target).parent().children().first().val();
								tipoConsulta = "eliminar";
								enviarDatos(codigo, tipoConsulta);
