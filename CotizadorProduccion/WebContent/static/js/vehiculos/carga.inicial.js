...
function cargar() {
 ...
 QueryString = function () {
	...
	$("#codigo_descuento").change(function () {
		...
		var min = $("#codigo_descuento option:selected").attr("minimo");//Number(aux.split('al')[0].trim());
		var max = $("#codigo_descuento option:selected").attr("maximo");//Number(aux.split(' al ')[1].trim());
	...
	$("#wizard").steps({
		...
		onStepChanging : function (event, currentIndex, newIndex) {
			...
			if (newIndex === 3) {											
				...
				// Agregamos el valor de la suma asegurada al monto del beneficiario								
				var valorEndosoBeneficiario = Number($("#valor_endoso_beneficiario").val());
				if(valorEndosoBeneficiario==0){
					$("#valor_endoso_beneficiario").val($("#total_suma_asegurada_vh").val());					
				}								
			}
...
function cargarPuntosVenta(seleccionada) {
	var productoValor = $("#productos").val();
	...
	$.ajax({
		...
		data : {
			...
			"grupoPorProductoId" : productoValor,

		},
		type : 'post',
		datatype : 'json',
		success : function (data) {
			var puntosVenta = data.puntosVenta;
			if(puntosVenta.length!=0){	
			$(".mensaje_no_producto").hide();
			...
			if(puntosVenta.length==0 && tipoObjeto=="VHDinamico" ){
				$(".content").hide();
 ...
 * QUE RECIBE.
 */
function cargarAgentes(seleccionada, valorComision,seleccionadaAgenteAcuerdoId) {
	$.ajax({
		...
		success : function (data) {
			...
			if (seleccionada != "") {
				if(seleccionadaAgenteAcuerdoId != "")
					obtenerAgenteComision(valorComision,seleccionadaAgenteAcuerdoId);
				else
 ...
 * QUE RECIBE.
 */
function cargarAgentesSelect2(seleccionada, valorComision,seleccionadaAgenteAcuerdoId) {
	$.ajax({
		url : '../AgenteController',
		data : {
		...
		success : function (data) {
			...
			var agentes = data.listadoAgentes;
			...
			$('#agentes').select2("val",seleccionada);
		...
		success : function (data) {
			...
			if (tipo != '') {
				if (noChange) {
					...
					$("#tipo_identificacion_" + tipo).val(seleccionada);
				} else {
 ...
 * SETEA EL VALOR QUE CORRESPONDE.
 */
function cargarDescuentos(seleccionada, tasa) {
	$.ajax({
	...
	$.ajax({
		...
		success : function (datos) {
			...
			if (tipo == "")
				...
				$("#provincia_" + tipo).val(seleccionada);
			}				
		}
	});
...
function cargarDatosVehiculo(valor, anio, antiguedad, edad, hijos, zona, dispositivo, cero_kilometros, genero, estado_civil, guarda_garage, combustible, sucursal, marca, modelo, color, tipoUso, tipoVehiculo, tipoAdquisicion, tasaProductoId,hijos_conduzcan,kilometraje_anual,zona_transito) {
	var numeroVeces = Number(valor);
	var options = '';
	var anoFabricacion = '';
	var anoActual = '';
	...
	$.ajax({
		...
		success : function (data) {
			...
			options = '<option value="">&nbsp;&nbsp;&nbsp;&nbsp;Seleccione&nbsp;&nbsp;&nbsp;&nbsp;</option>';
			if(tarificador=="V2"){
				options += '<option value="J">&nbsp;&nbsp;P.Juridica</option>';
			}				
			for (var i = edadMinima; i < edadMaxima + 1; i++) {
				if(tarificador=="V2"){
					if(i!=80){
						...
						options += '<option value="' + i + '">' + "80+" + '</option>';
					}	
				}else if(tarificador=="V1"){
					options += '<option value="' + i + '">' + i + '</option>';
				}
			...
			if(tarificador=="V2"){
				...
				if ($("#porcentaje_suma_aseguradaDT").val() == "")
					$("#porcentaje_suma_aseguradaDT").val(Number(data.deducible_perdida_total) * Number('100'));
				...
				if ($("#deducible_rc").val() == "")
					$("#deducible_rc").val(Number(data.deducible_rc));								
			}
			...
			if (numeroVeces == 1) {
				...
				$("#deducible_perdida_total_defecto").val(Number(data.deducible_perdida_total) * Number('100'));
				$("#deducible_rc_defecto").val(Number(data.deducible_rc));			
			}
			...
			if(tarificador=="V2"){
				...
				// Consultar Zonas de Transito
				$.ajax({
					url : '../TarificadorController',
					data : {
						"tipoConsulta" : "encontrarTodosVhTari1Zona"						
					},
					type : 'post',
					datatype : 'json',
					success : function (data) {
						var listadoZonas = data.listadoVhTari1Zona;
						$("#zona_transito" + numeroVeces).html("");
						$("#zona_transito" + numeroVeces).append("<option value=''>&nbsp;&nbsp;&nbsp;&nbsp;Seleccione&nbsp;&nbsp;&nbsp;&nbsp;</option>");
						
						$.each(listadoZonas, function (index) {
							$("#zona_transito" + numeroVeces).append("<option value='" + listadoZonas[index].id + "' >" + listadoZonas[index].nombre + "</option>");
						});
						$("#zona_transito" + numeroVeces.toString()).val(zona_transito);						
					}
				});
				
				
				options = '';
				// Agregamos Kilometraje Anual
				options = '<option value="">&nbsp;&nbsp;&nbsp;&nbsp;Seleccione&nbsp;&nbsp;&nbsp;&nbsp;</option>';
				options += '<option value="8000">&nbsp;&nbsp;8000 </option>';
				options += '<option value="15000">&nbsp;&nbsp;15000 </option>';
				options += '<option value="20000">&nbsp;&nbsp;20000 </option>';
				options += '<option value="25000">&nbsp;&nbsp;25000 </option>';
				options += '<option value="30000">&nbsp;&nbsp;30000 </option>';
				options += '<option value="40000">&nbsp;&nbsp;40000 </option>';
				options += '<option value="45000">&nbsp;&nbsp;45000 </option>';
				$("#kilometraje_anual" + numeroVeces.toString()).html(options).val(kilometraje_anual);	
				
				options = '';
				// Agregamos Hijos hasta 25 anios
				options = '<option value="">&nbsp;&nbsp;&nbsp;&nbsp;Seleccione&nbsp;&nbsp;&nbsp;&nbsp;</option>';
				options += '<option value="SI">&nbsp;&nbsp;SI </option>';
				options += '<option value="NO">&nbsp;&nbsp;NO </option>';				
				$("#hijos_conduzcan" + numeroVeces.toString()).html(options).val(hijos_conduzcan);
