...
function formatDollar(num) {
	...
	return "$"
			+ p[0].split("").reverse().reduce(function(acc, numero, i, orig) {
...
function distinct(num) {
	...
	for (var i = 0; i < data.length; i++) {
		obj = data[i];
		for ( var key in obj) { // trae la clave unica
			if (obj.hasOwnProperty(key)) {
				if (!tempMap.hasOwnProperty(key + obj[key])) { // no esta en el
																// mapa
					tempMap[key + obj[key]] = obj; // este se añade en el mapa
					distinct.push(obj); // add en la lista sin son distintos
				}
				break;
			}
		}
...
function onSelect(e) {
	if ($(e.item).find("> .k-link").text().trim() == "Coberturas") {
		if ($("#total_valor_asegurado").val() != ""
			...
			$("#valor_estructura").prop("disabled", true);
			$("#valor_contenidos").prop("disabled", true);
			if (cotizacionEstado == "Pendiente Asignar Inspector"
					|| cotizacionEstado == "Pendiente de Inspeccion"
					|| cotizacionEstado == "Pendiente"
					|| cotizacionEstado == "Emitido")
...
function limpiarCoberturas() {
	if (confirm("El cambio de valores afecta a las coberturas ya establecidas. Desea continuar?")) {
		...
		// var numerictextbox1 =
		// $("#valor_estructuras").data("kendoNumericTextBox");
		// numerictextbox1.enable(true);
		// var numerictextbox2 =
		// $("#valor_muebles_enseres").data("kendoNumericTextBox");
		// numerictextbox2.enable(true);
		// var numerictextbox3 =
		// $("#valor_maquinarias").data("kendoNumericTextBox");
		// numerictextbox3.enable(true);
		// var numerictextbox4 =
		// $("#valor_mercaderia").data("kendoNumericTextBox");
		// numerictextbox4.enable(true);
		// var numerictextbox5 =
		// $("#valor_herramientas").data("kendoNumericTextBox");
		// numerictextbox5.enable(true);
		// var numerictextbox6 =
		// $("#valor_insumos_medicos").data("kendoNumericTextBox");
...
function obtenerConfiguracionesCoberturaProducto(productoID) {
	...
	$
			.ajax({
				...
				data : {
					"tipoConsulta" : "encontrarConfiguracionesPorProductoPlanId",
				...
				success : function(data) {
					...
					var filterCoberturasDependen = $(configCobertasPorProducto)
							.filter(
									function(idx) {
										return configCobertasPorProducto[idx].tipoDeclaracion == 1;
									});

					var productos = distinct(filterCoberturasDependen);
					$
							.each(
									productos,
									function(index) {
										$("#coberturas_generales")
												.append(
														'<tr><td style="color: green; width: 20%"><input type="checkbox"  disabled="" id="cobertura_'
																+ productos[index].configuracionCoberturaId
																+ '" class="datosPymes coberturaPorDireccion" onclick=crearCobertura('
																+ productos[index].configuracionCoberturaId
																+ ',"general"'
																+ ','
																+ productos[index].planId
																+ ')>'

																+ productos[index].coberturaNombre
																+ '</td>'

																+ '<td style="width: 40%">'
																+ '<div id="planes_general_cobertura_'
																+ productos[index].configuracionCoberturaId
																+ '"class="btn-group alignment Generales"  data-toggle-name="selectGenerales'
																+ productos[index].configuracionCoberturaId
																+ '" data-toggle="buttons-checkbox">'
																+ '</div>'
																+ '<input id="codigoGenerales'
																+ productos[index].configuracionCoberturaId
																+ '" type="hidden" name="selectGenerales'
																+ productos[index].configuracionCoberturaId
																+ '" value="0">'
																+ '</td>'
																+ '<td style="width: 40%; vertical-align: top;">'
																+ '<table id="detalle_cobertura_general_'
																+ productos[index].configuracionCoberturaId
																+ '" class="datosPymes" >'
																+ '</table></td>'
																+ '</tr>');

										cargarPlanGeneralesGroup(
												productos[index].configuracionCoberturaId,
												productos[index].dependeValor);
										$(
												'div[data-toggle="buttons-checkbox"] .btng'
														+ productos[index].configuracionCoberturaId)
												.click(
														function() {
															$(
										...
										$(
												...
												.click(
														function() {
															...
															calcularTotalValorAseguradoPlanGenerales(productos[index].configuracionCoberturaId);
														});

										$('div.btn-group[data-toggle-name]')
												.each(
														function() {
															var group = $(this);
															var form = group
...
function crearCobertura(configuracionCoberturaId, tipo, plan) {
	var filterResult1 = $(configCobertasPorProducto)
			.filter(
					function(idx) {
						return (configCobertasPorProducto[idx].configuracionCoberturaId == configuracionCoberturaId && configCobertasPorProducto[idx].planId == plan);
					});
	if (filterResult1.length > 0) {
		if ($("#cobertura_" + configuracionCoberturaId).is(':checked')) {
			var valorMaximo = filterResult1[0].valorAsegurado;
			var tasa = determinarTasa(configuracionCoberturaId);
			var clase = filterResult1[0].incluyeEnProducto == "1" ? "predeterminado"
					: "normal";
			var formacionFila = '<tr class="' + clase
					+ '" id="fila_config_cobertura_' + configuracionCoberturaId
					+ '">' + '<td style="width: 50%">' + '<b>'
					+ filterResult1[0].coberturaNombre + '</b>'
					+ '<div class="panel panel-primary">'
					+ '<table style="width: 100%" cellpadding="3">';
			formacionFila = formacionFila
					+ '<tr>'
					+ '<td><label class="porcentajeTasasPymes" id="lbl_tasa_sugerida_'
					+ configuracionCoberturaId
					+ '">Tasa Sugerida:</label></td>'
					+ '<td><input type="text" disabled ="" style="width: 70px" disabled id="tasa_sugerida_'
					+ configuracionCoberturaId
					...
					+ '<td><input type="text" disabled ="" style="width: 70px" id="tasa_ingresada_'
					+ configuracionCoberturaId
					...
					+ '<td><input type="text" disabled ="" style="width: 100px" id="monto_asegurado_'
					+ configuracionCoberturaId;
			if (tipo == "direccionContenidos" || tipo == "direccionEstructuras")
				formacionFila = formacionFila
						+ '" class="datosPymes datosMontoAseguradoDir" onchange="calcularPrimaNeta(';
			else
				formacionFila = formacionFila
						+ '" class="datosPymes datosMontoAseguradoGen" onchange="calcularPrimaNeta(';
			formacionFila = formacionFila
					+ configuracionCoberturaId
					+ ','
					+ valorMaximo
					...
					+ ')"></td>'
					+ '<td>Prima Neta:</td>'
					+ '<input type="hidden"  id="planId_'
					+ configuracionCoberturaId
					+ '" value="'
					+ plan.trim()
					+ '">'
					+ '<input type="hidden" class="cobertura" id="codigoCobertura_'
					+ configuracionCoberturaId
					+ '" value="'
					+ configuracionCoberturaId
					+ '" >'
					+ '<input type="hidden" id="nombreCobertura_'
					+ configuracionCoberturaId
					+ '" value="'
					+ filterResult1[0].coberturaNombre
					+ '">'
					+ '<td><input type="text" disabled ="" style="width: 100px" id="prima_neta_'
					+ configuracionCoberturaId;
			if (tipo == "direccionContenidos" || tipo == "direccionEstructuras")
				formacionFila = formacionFila
				...
				formacionFila = formacionFila
						+ '" class="datosPymes" disabled></td>';
			formacionFila = formacionFila + '</tr>';

			formacionFila = formacionFila
					+ '<tr>'
					+ '<td colspan="8" style="color: blue; font-size: x-small;">'
					+ '<label id="advertencia_' + configuracionCoberturaId
					+ '"></label>' + '</td>' + '</tr>' + '</table>' + '</div>'
					+ '</td>' + '</tr>';
			if (tipo == "direccionContenidos" || tipo == "direccionEstructuras")
				$("#detalle_cobertura_" + tipo).append(formacionFila);
			else
				$("#detalle_cobertura_" + tipo + "_" + configuracionCoberturaId)
						.append(formacionFila);

			var tipoVariable = $("#seguridad_adecuada_si").is(":checked") ? "true"
					: "false";

			cargarDeducibles(configuracionCoberturaId, tipoVariable,
					filterResult1[0].coberturaNombre);

			$("#tasa_sugerida_" + configuracionCoberturaId).val(tasa);
			$("#tasa_ingresada_" + configuracionCoberturaId).val(tasa);
			$("#monto_asegurado_" + configuracionCoberturaId).val(valorMaximo);
			calcularPrimaNeta(configuracionCoberturaId, valorMaximo, plan);
...
function cargarCobertura(configuracionCoberturaId, tipo, tasaSugerida,
		tasaIngresada, montoAsegurado, valorPrima, valorAdicional, plan) {
			...
			.filter(
					function(idx) {
						return configCobertasPorProducto[idx].configuracionCoberturaId == configuracionCoberturaId
								&& configCobertasPorProducto[idx].planId == plan;
					});
	if (filterResult.length > 0) {

		$("#cobertura_" + configuracionCoberturaId).prop("checked", true);
		$("#coberturas_por_" + tipo)
				.append(
						'<tr><td style="color: green;"><input type="checkbox" checked="" disabled="" id="cobertura_'
								+ filterResult[0].configuracionCoberturaId
								+ '" class="datosPymes coberturaPorDireccion" onclick=crearCobertura('
								+ filterResult[0].configuracionCoberturaId
								+ ',"direccion"'
								+ ','
								+ filterResult[0].planId
								+ ')>'

								+ filterResult[0].coberturaNombre
								+ '</td><td style="color: green;">('+filterResult[0].ramoNombre+')</td></tr>');
		var valorMaximo = filterResult[0].valorAsegurado;
		var clase = filterResult[0].incluyeEnProducto == "1" ? "predeterminado"
				: "normal";
		var FormacionFila = '<tr class="' + clase
				+ '" id="fila_config_cobertura_' + configuracionCoberturaId
				+ '">' + '<td style="width: 50%">' + '<b>'
				+ filterResult[0].coberturaNombre + '</b>'
				+ '<div class="panel panel-primary">'
				+ '<table style="width: 100%" cellpadding="3">';
		FormacionFila = FormacionFila
				+ '<tr>'
				...
				+ '<td><input type="text" disabled="" style="width: 100px" id="prima_neta_'
				+ configuracionCoberturaId;
		if (tipo == "direccionEstructuras" || tipo == "direccionContenidos")
			FormacionFila = FormacionFila
					+ '" class="datosPymes primaNetaCobertura" disabled></td>';
		else
			FormacionFila = FormacionFila
					+ '" class="datosPymes" disabled></td>';
		FormacionFila = FormacionFila + '<td></td>' + '<td></td>' + '</tr>';

		FormacionFila = FormacionFila + '<tr>'
				+ '<td colspan="6" style="color: blue; font-size: x-small;">'
				+ '<label id="advertencia_' + configuracionCoberturaId
				+ '"></label>' + '</td>' + '</tr>' + '</table>' + '</div>'
				+ '</td>' + '</tr>';
		...
		if (filterResult[0].usaValorAdicional == "1")
			$("#valor_adicional_" + configuracionCoberturaId).val(
					valorAdicional);
...
function cargarCoberturaGeneral(configuracionCoberturaId, tipo, tasaSugerida,
		tasaIngresada, montoAsegurado, valorPrima, plan) {
	var filterResult = $(configCobertasPorProducto)
			.filter(
					function(idx) {
						return configCobertasPorProducto[idx].configuracionCoberturaId == configuracionCoberturaId
	...
	if (filterResult.length > 0) {
		...
		FormacionFila = FormacionFila
				...
				+ configuracionCoberturaId
				+ '"></label></td>'
				+ '<td><input type="hidden" disabled="" style="width: 70px" disabled id="tasa_sugerida_'
				+ configuracionCoberturaId
				+ '" class="datosPymes porcentajeTasasPymes"></td>'
				+ '<td><label class="porcentajeTasasPymes" id="lbl_tasa_ingresada_'
				+ configuracionCoberturaId
				+ '"></label></td>'
				+ '<td><input type="hidden" disabled="" style="width: 70px" id="tasa_ingresada_'
				+ configuracionCoberturaId
				+ '" class="datosPymes porcentajeTasasPymes" onchange="calcularPrimaNeta('
				+ configuracionCoberturaId + ',' + valorMaximo + ')"></td>'
				+ '<td></td>' + '<td></td>' + '<td></td>' + '<td></td>'
				+ '</tr>' + '<tr>';
		if (filterResult[0].dependeValor == "10") {
			FormacionFila = FormacionFila
					+ '<td>Valor Asegurado:</td>'
					+ '<td><input type="text" style="width: 100px" id="monto_asegurado_';
		} else {
			FormacionFila = FormacionFila
					+ '<td><input type="hidden" disabled="" style="width: 100px" id="monto_asegurado_';
		}
		FormacionFila = FormacionFila
				+ configuracionCoberturaId
				+ '" class="datosPymes" onchange="calcularPrimaNeta('
				...
				+ plan.trim()
				+ '">'
				+ '<input type="hidden"  id="codigoCobertura_'
				+ filterResult[0].configuracionCoberturaId
				+ '" value="'
				+ filterResult[0].configuracionCoberturaId
				+ '" >'
...
function mostrarResumen() {
	$
			.ajax({
				...
				datatype : 'json',
				success : function(data) {
					$("#resumen_inspeccion_total_suma_asegurada").val(
							formatDollar(data.valorAsegurado));
					$("#resumen_inspeccion_prima_sin_impuestos").val(
							formatDollar(data.valorPrima));
					$("#resumen_inspeccion_super_bancos").val(
							formatDollar(data.valorSuperBancos));
					if (parseFloat(data.porcentajeDescuento) > 0)
						$("#resumen_inspeccion_filaDescuento").show();
					$("#resumen_inspeccion_porcentaje_descuento").val(
							formatDollar(data.porcentajeDescuento));
					$("#resumen_inspeccion_seguro_campesino").val(
							formatDollar(data.valorSeguroCampesino));
					$("#resumen_inspeccion_derecho_emision").val(
							formatDollar(data.valorDerechosEmision));
					$("#resumen_inspeccion_subtotal").val(
							formatDollar(data.valorSubTotal));
					$("#resumen_inspeccion_iva").val(
							formatDollar(data.valorIva));
					$("#resumen_inspeccion_total").val(
							formatDollar(data.valorTotal));
					$("#resumen_total_suma_asegurada").val(
							formatDollar(data.valorAsegurado));
					$("#resumen_prima_sin_impuestos").val(
							formatDollar(data.valorPrima));
					$("#resumen_super_bancos").val(
							formatDollar(data.valorSuperBancos));
					if (parseFloat(data.porcentajeDescuento) > 0)
						$("#resumen_filaDescuento").show();
					$("#resumen_porcentaje_descuento").val(
							formatDollar(data.porcentajeDescuento));
					$("#resumen_derecho_emision").val(
							formatDollar(data.valorDerechosEmision));
					$("#resumen_subtotal")
							.val(formatDollar(data.valorSubTotal));
...
function cargarDeducible(configuracionCoberturaId, textDeducible, ids, valores,
		textosDeducible, planId) {
	var filterResult = $(configCobertasPorProducto)
			.filter(
					function(idx) {
						return configCobertasPorProducto[idx].configuracionCoberturaId == configuracionCoberturaId;
			...
			$("#deducibles_por_direccion")
					.append(
							'<tr id="deducible_cobertura_'
									...
									+ filterResult[0].coberturaNombre
									+ '<input type="hidden" class="deducibleIDsDir" id="deducible_cobertura_'
									+ configuracionCoberturaId
									+ '" value="'
									+ configuracionCoberturaId
									+ '"></td></tr><tr>'
									...
									+ configuracionCoberturaId + '" value="'
									+ textosDeducible
									+ '"></td></tr></table></td></tr>');
		}
	}
...
function cargarDeducibleGeneral(configuracionCoberturaId, textDeducible, ids,
		valores, textosDeducible, planId) {
			...
			$("#deducibles_general")
					.append(
							'<tr id="deducible_cobertura_'
									...
									+ '<td><table><tr><td style="font-weight:bold">Deducible:&nbsp;'
									+ filterResult[0].coberturaNombre
									+ '<input type="hidden" class="deducibleIDs" id="deducible_cobertura_'
									+ configuracionCoberturaId
									+ '" value="'
									+ configuracionCoberturaId
									+ '"></td></tr><tr>'
...
function cargarAsistencia(asistenciaId, valor) {
	// Filtro en el array de cobertura el id seleccionado
	var filterResult = $(configAsistenciasPorProducto).filter(function(idx) {
...
function determinarMaximoGeneral() {
	var valorMaximoIncendio = 0;
	$.ajax({
		url : '../PymesObjetoCotizacionController',
		data : {
			"tipoConsulta" : "obtenerMaximoAseguradoIncendio",
		...
		success : function(data) {
			if (data.success) {
				valorMaximoIncendio = data.valorAsegurado;
			}
		}
	});
	return valorMaximoIncendio;
}
...
function determinarValorMaximo(configuracionCoberturaId, planId) {
	// Filtro en el array de cobertura el id seleccionado
	var filterResult = $(configCobertasPorProducto)
			.filter(
					function(idx) {
						return (configCobertasPorProducto[idx].configuracionCoberturaId == configuracionCoberturaId && configCobertasPorProducto[idx].configuracionCoberturaId == planId);
					});
	if (filterResult.length > 0) {
...
function calcularPrimaNeta(configuracionCoberturaId, montomaximo, planId,
		dependeValor) {
	var p2 = Number($("#monto_asegurado_" + configuracionCoberturaId).val());
	if (dependeValor == "10") {
		var tasaD = determinarTasa(configuracionCoberturaId, p2);
		$("#tasa_sugerida_" + configuracionCoberturaId).val(tasaD);
		$("#tasa_ingresada_" + configuracionCoberturaId).val(tasaD);
	}
	...
	if (montomaximo) {
		if (montomaximo != -1 && montomaximo != 0) {
			if (p2 > montomaximo) {
				...
				$("#monto_asegurado_" + configuracionCoberturaId).val(
						montomaximo);
				p2 = montomaximo;
			}
		}
	...
	var filterResult = $(configCobertasPorProducto)
			.filter(
					function(idx) {
						return (configCobertasPorProducto[idx].configuracionCoberturaId == configuracionCoberturaId && configCobertasPorProducto[idx].planId == planId);
					});
...
function calcularTotalValorAsegurado() {
	...
	numerictextbox.value(valEst + valMue + valMaq + valMer + valHer + valIns);

	// var tabStrip = $("#tabstrip").kendoTabStrip().data("kendoTabStrip");
	if (Number($("#total_valor_asegurado").val()) > Number(parametroGrupoProducto.limiteAsegurado)) {
		$("#advertencia_general")
				.html(
						"El l&iacute;mite asegurado de incendio por cotizaci&oacute;n no puede ser mayor a: "
								+ formatDollar(parametroGrupoProducto.limiteAsegurado));
		// tabStrip.enable(tabStrip.tabGroup.children().eq(2), false);
	} else {
		...
		var filterCoberturasDependen = $(configCobertasPorProducto).filter(
				function(idx) {
					return configCobertasPorProducto[idx].dependeValor != "1";
				});
		$
				.each(
						filterCoberturasDependen,
						function(index) {
							...
							case 2: // De Estructuras
								if (valEst > 0)
									$(
											"#cobertura_"
													+ filterCoberturasDependen[index].configuracionCoberturaId)
											.prop("disabled", false);
								else
									$(
											"#cobertura_"
		...
		// Busco todas las coberturas que son predeterminadas para generarlas
		// automáticamente
		var filterResult = $(configCobertasPorProducto).filter(function(idx) {
				...
				.each(
						...
						function(index) {
							...
							if (filterResult[index].tipoDeclaracion == "2") {
								// determina la tasa de acuerdo al id de la
								// configuración
								...
								if (tasa > 0) {
									...
									crearCobertura(
											filterResult[index].configuracionCoberturaId,
											"direccion");
									if (Number($("#total_valor_asegurado")
											.val()) > Number(valorMaximo))
										$(
												"#monto_asegurado_"
														+ filterResult[index].configuracionCoberturaId)
												.val(Number(valorMaximo));
											...
											$("#deducibles_general")
													.append(
															'<tr id="deducible_cobertura_'
																	...
																	+ filterResult[index].configuracionCoberturaId
																	+ '" value="'
																	+ filterResult[index].idsDeducible
																	+ '"><input type="hidden" id="valores_deducible_cobertura_'
																	+ filterResult[index].configuracionCoberturaId
																	+ '" value="'
																	+ filterResult[index].valoresDeducible
																	+ '"><input type="hidden" id="textos_deducible_cobertura_'
																	+ filterResult[index].configuracionCoberturaId
																	+ '" value="'
																	+ filterResult[0].textosDeducible
																	+ '"></td></tr></table></td></tr>');
										}
									}
								}
...
function grabarDireccion() {
	...
	var valor1 = $("#planes_contenidos option:selected").html();
	if (valor1 == undefined)
		valor1 = 'SIN PLAN';
	var resContenidos = valor1.substring(0, 7).trim();
	var valor2 = $("#planes_estructuras option:selected").html();
	if (valor2 == undefined)
		valor2 = 'SIN PLAN';
	var resEstructuras = valor2.substring(0, 7).trim();
	...
	var filterCoberturasEstructuras = $(configCobertasPorProducto)
			.filter(
					function(idx) {
						return ((configCobertasPorProducto[idx].tipoDeclaracion == "3" || configCobertasPorProducto[idx].dependeValor == "2") && configCobertasPorProducto[idx].nombrePlan == resEstructuras);
					});

	var filterCoberturasContenidos = $(configCobertasPorProducto)
			.filter(
					function(idx) {
						return ((configCobertasPorProducto[idx].tipoDeclaracion == "4" || configCobertasPorProducto[idx].dependeValor == "8") && configCobertasPorProducto[idx].nombrePlan == resContenidos);
					});
	...
	$
			.each(
					filterCoberturasEstructuras,
					function(index) {
						if ($(
								"#cobertura_"
										+ filterCoberturasEstructuras[index].configuracionCoberturaId)
								.is(':checked')) {
							var tasaSugerida = 0;
							var tasaIngresada = 0;
							...
							var planGeneral = 0;
							if ($("#tasa_sugerida_"
									+ filterCoberturasEstructuras[index].configuracionCoberturaId).length)
								tasaSugerida = Number($(
										"#tasa_sugerida_"
												+ filterCoberturasEstructuras[index].configuracionCoberturaId)
										.val());
							...
							if ($("#valor_adicional_"
								...
								if ($("#valor_adicional_"
									...
									valorAdicional = Number($(
											...
											.val());
							var cobertura = {
								configuracionCoberturaId : filterCoberturasEstructuras[index].configuracionCoberturaId,
	...
	$(".deducibleIDsDir").each(
			function() {
				var deducibleId = $(this).val();
				var textoDeducible = $(
						"#text_area_deducible_cobertura_" + deducibleId).val();
				var idsDeducible = $("#ids_deducible_cobertura_" + deducibleId)
						.val();
				var valoresDeducible = $(
						"#valores_deducible_cobertura_" + deducibleId).val();
				var textosDeducible = $(
						"#textos_deducible_cobertura_" + deducibleId).val();
				var cobertura = {
	...
	$
			.each(
					filterCoberturasContenidos,
					function(index) {
						if ($(
								"#cobertura_"
										+ filterCoberturasContenidos[index].configuracionCoberturaId)
								.is(':checked')) {
							var tasaSugerida = 0;
							var tasaIngresada = 0;
							var montoAsegurado = 0;
							...
							var planGeneral = 0;
							if ($("#tasa_sugerida_"
									+ filterCoberturasContenidos[index].configuracionCoberturaId).length)
								tasaSugerida = Number($(
										"#tasa_sugerida_"
												+ filterCoberturasContenidos[index].configuracionCoberturaId)
										.val());
							...
							if ($("#valor_adicional_"
								...
								if ($("#valor_adicional_"
									...
									valorAdicional = Number($(
											...
											.val());
							var cobertura = {
								configuracionCoberturaId : filterCoberturasContenidos[index].configuracionCoberturaId,
								tasaSugerida : tasaSugerida,
								tasaIngresada : tasaIngresada,
								valorIngresado : montoAsegurado,
								primaCalculada : primaCalculada,
	...
	$
			.ajax({
				...
				data : {
					...
					"provinciaId" : $(
							"#ubicacion_provincia_" + direccionActualId).val(),
					"cantonId" : $("#ubicacion_canton_" + direccionActualId)
							.val(),
					"callePrincipal" : $(
							"#ubicacion_calle_principal_" + direccionActualId)
							.val(),
					"numeroDireccion" : $(
							"#ubicacion_numero_" + direccionActualId).val(),
					"calleSecundaria" : $(
							"#ubicacion_calle_secundaria_" + direccionActualId)
							.val(),
					"actividadEconomicaId" : $(
							"#actividad_economica option:selected").val(),
					"tipoPredioId" : $("#giro_negocio option:selected").val(),
					"tieneMasDosAnios" : "true",
					"contabilidadFormal" : "true",
					"requiereInspeccion" : $("#requiere_inspeccion_si").is(
							":checked") ? "true" : "false",
					"seguridadAdecuada" : $("#seguridad_adecuada_si").is(
							":checked") ? "true" : "false",
					"sector" : $("#ubicacion_sector").val(),
					"nombreEdificio" : $("#ubicacion_nombre_edificio").val(),
					"numeroOficina" : $("#ubicacion_numero_oficina").val(),
					"valorEstructuras" : $("#valor_estructura").val(),
					"valorMueblesEnseres" : $("#valor_muebles_enseres").val(),
					"valorMaquinaria" : $("#valor_maquinarias").val(),
					"valorMercaderia" : $("#valor_mercaderia").val(),
					"valorInsumosNoelectronicos" : $("#valor_insumos_medicos")
							.val(),
					"planesEstructuras" : $("#planes_estructuras").val(),
					"planesContenidos" : $("#planes_contenidos").val(),
				...
				success : function(data) {
					if (data.success) {
						$("#cotizacionDetalleId_" + direccionActualId).val(
...
function cargarPestanaEndosoBeneficiario(monto, beneficiario, rubro,
		endosoBeneficiarioId) {
	...
	$('#endoso_beneficiario_id_' + endosoBeneficiarioId).val(
			endosoBeneficiarioId);
}
	...
	$.ajax({
		...
		success : function(data) {
			...
			actividadEconomicaList
					.setDataSource(data.listadoActividadesEconomicas);
		}
	});
	...
	$
			.ajax({
				...
				data : {
					...
					"provincia" : $("#ubicacion_provincia_" + codigoDireccion)
							.val()
				},
				async : false,
				type : 'post',
				...
				success : function(data) {
					...
					$("#ubicacion_canton_" + codigoDireccion).append(
					...
					$
							.each(
									...
									function(index) {
										...
										if ($("#productos").val() == "212") {
											if ($(
													"#ubicacion_provincia_"
															+ codigoDireccion)
													.val() == '9') {
...
function obtenerConfiguracionDireccion(numeroDireccion) {
	if ($("#ubicacion_provincia_" + numeroDireccion).val() == undefined
			|| $("#ubicacion_provincia_" + numeroDireccion).val() == "") {
	...
	if ($("#ubicacion_canton_" + numeroDireccion).val() == undefined
			|| $("#ubicacion_canton_" + numeroDireccion).val() == "") {
		alert("Debe seleccionar un cantón.");
		return;
	}
	if ($("#ubicacion_calle_principal_" + numeroDireccion).val() == undefined
			|| $("#ubicacion_calle_principal_" + numeroDireccion).val() == "") {
		alert("Debe ingresar la calle principal.");
		return;
	}
	if ($("#ubicacion_numero_" + numeroDireccion).val() == undefined
			|| $("#ubicacion_numero_" + numeroDireccion).val() == "") {
		alert("Debe ingresar el numero.");
		return;
	}
	if ($("#ubicacion_calle_secundaria_" + numeroDireccion).val() == undefined
			|| $("#ubicacion_calle_secundaria_" + numeroDireccion).val() == "") {
		alert("Debe ingresar la calle secundaria.");
		return;
	}

	limpiarControles();
	cargarActividadesEconomicas($("#ProductoID").val());
	$("#numeroDireccion").val(numeroDireccion);
	$("#cotizacionDetalleId").val(
			$("#cotizacionDetalleId_" + numeroDireccion).val());
	if ($("#cotizacionDetalleId").val() != "") {
		$
				.ajax({
					url : '../PymesObjetoCotizacionController',
					data : {
						"tipoConsulta" : "obtenerConfiguracionPorDetallePlanId",
						"cotizacionDetalleId" : $("#cotizacionDetalleId").val()
					},
					async : false,
					type : 'post',
					...
					success : function(data) {
						...
						cargarPlanEstructuras();
						cargarPlanContenidos();
						cargarPlanEstructurasGroup();
						cargarPlanContenidosGroup();
						...
						$("#actividad_economica").data("kendoMultiSelect")
								.value(data.actividadEconomicaId);
						$("#planes_estructuras").val(data.planEstructuraId);
						$("#planes_contenidos").val(data.planContenidosId);
						$("#codigoEstructura").val(data.planEstructuraId);
						$("#codigoContenido").val(data.planContenidosId);

						$('div[data-toggle="buttons-checkbox"] .btnc').click(
								function() {
									$(
											'input[name="'
													+ $(this).parent().attr(
															'id') + '"]').val(
											$(this).val());
									$("#planes_contenidos").val($(this).val());
								});
						$('div[data-toggle="buttons-checkbox"] .btne')
								.click(
										function() {
											$(
													'input[name="'
						...
						$(".Contenido .btnc")
								.click(
										function() {
											$(".Contenido .btnc").removeClass(
													"active");
											$(this).addClass("active");
											calcularTotalValorAseguradoPlanContenidos(
													$(this).val(), $(this)
															.text(), data.rol);
										});

						$(".Estructura .btne").click(
								function() {
									$(".Estructura .btne")
											.removeClass("active");
									$(this).addClass("active");
									calcularTotalValorAseguradoPlanEstructuras(
											$(this).val(), $(this).text(),
											data.rol);
								});
						...
						var resContenidos = $(
								"#planes_contenidos option:selected").val();
						var resEstructuras = $(
								"#planes_estructuras option:selected").val();
						cargarCamposOcultos();
						$("#planes_estructuras").hide();
						$("#planes_contenidos").hide();
						...
						if ($("#ProductoID").val() == "190") { // Quemado que
																// sea solo para
																// edificios
							$("#fila_constitucion").hide();
						}
						$("#ubicacion_sector").val(data.sector);
						$("#ubicacion_nombre_edificio")
								.val(data.nombreEdificio);
						$("#ubicacion_numero_oficina").val(data.numeroOficina);
						var p1 = Number($("#valor_estructura").val());
						var p2 = Number($("#valor_contenidos").val());
						var numerictextbox = $("#total_valor_asegurado").data(
								"kendoNumericTextBox");
						numerictextbox.value(p1 + p2);
						$("#detalle_cobertura_direccionEstructuras").empty();
						$("#coberturas_por_direccionEstructuras").empty();
						$("#detalle_cobertura_direccionContenidos").empty();
						$("#coberturas_por_direccionContenidos").empty();
						$("#deducibles_por_direccion").empty();

						var filterResultEstructuras = $(coberturasConfiguradas)
								.filter(
										function(idx) {
											return coberturasConfiguradas[idx].direccion == "direccionEstructuras";
										});
						$
								.each(
										filterResultEstructuras,
										function(index) {
											cargarCobertura(
													filterResultEstructuras[index].configuracionCoberturaId,
													filterResultEstructuras[index].direccion,
													filterResultEstructuras[index].tasaSugerida,
													filterResultEstructuras[index].tasaIngresada,
													filterResultEstructuras[index].valorIngresado,
													filterResultEstructuras[index].primaCalculada,
													filterResultEstructuras[index].valorAdicional,
													resEstructuras);

										});
						var filterResultContenidos = $(coberturasConfiguradas)
								.filter(
										function(idx) {
											return coberturasConfiguradas[idx].direccion == "direccionContenidos";
										});

						$
								.each(
										filterResultContenidos,
										function(index) {
											cargarCobertura(
													filterResultContenidos[index].configuracionCoberturaId,
													filterResultContenidos[index].direccion,
													filterResultContenidos[index].tasaSugerida,
													filterResultContenidos[index].tasaIngresada,
													filterResultContenidos[index].valorIngresado,
													filterResultContenidos[index].primaCalculada,
													filterResultContenidos[index].valorAdicional,
													resContenidos);
						...
						$
								.each(
										coberturasDeducibles,
										function(index) {
											cargarDeducible(
													coberturasDeducibles[index].configuracionCoberturaId,
						...
						$("#prima_neta_por_direccion").text(
								formatDollar(valorTotal));
						$('#add').modal('show');
						$('div.btn-group[data-toggle-name]').each(function() {
							var group = $(this);
							var form = group.parents('form').eq(0);
							var name = group.attr('data-toggle-name');
							var hidden = $('input[name="' + name + '"]').val();
		...
		$("#planes_estructuras").val("-1");
		$("#planes_contenidos").val("-1");
		$("#codigoEstructura").val("-1");
		$("#codigoContenido").val("-1");
		$("#detalle_cobertura_direccionEstructuras").empty();
		$("#coberturas_por_direccionEstructuras").empty();
		$("#detalle_cobertura_direccionContenidos").empty();
		$("#coberturas_por_direccionContenidos").empty();
		...
		$('div[data-toggle="buttons-checkbox"] .btnc').click(
				function() {
					$('input[name="' + $(this).parent().attr('id') + '"]').val(
							$(this).val());
					$("#planes_contenidos").val($(this).val());
				});
		$('div[data-toggle="buttons-checkbox"] .btne').click(
				function() {
					$('input[name="' + $(this).parent().attr('id') + '"]').val(
							$(this).val());
					$("#planes_estructuras").val($(this).val());
				});

		$(".Contenido .btnc").click(
				function() {
					$(".Contenido .btnc").removeClass("active");
					$(this).addClass("active");
					calcularTotalValorAseguradoPlanContenidos($(this).val(), $(
							this).text(), rol);
				});

		$(".Estructura .btne").click(
				function() {
					$(".Estructura .btne").removeClass("active");
					$(this).addClass("active");
					calcularTotalValorAseguradoPlanEstructuras($(this).val(),
							$(this).text(), rol);
				});
		...
		$('#add').modal('show');
		if ($("#ProductoID").val() == "190") { // Quemado que sea solo para
												// edificios
			$("#fila_constitucion").hide();
		}
	}
	...
	$("#direcciones")
			.append(
					'<tr>'
							...
							+ '<input type="hidden" id="cotizacionDetalleId_'
							+ numeroDirecciones
							+ '"/>'
	...
	$(".eliminar-extra-btn").bind(
			{
				click : function() {
					...
					var numeroDirecciones = parseInt($("#contadorDirecciones")
							.val())
							- parseInt(1);
					$("#contadorDirecciones").val(numeroDirecciones);
				}
			...
			.ajax({
				...
				success : function(data) {
					if (data.success) {
						...
						if (datosCotizacion.estadoCotizacion == "Pendiente Asignar Inspector"
								|| datosCotizacion.estadoCotizacion == "Borrador") {
							$("#lblContenidoMensajeInspeccion")
									.html(
											"No puede continuar, porque debe realizar la inspecci&oacute;n.");
	...
	$.ajax({
		...
		success : function(data) {
			if (data.success) {
				...
				$("#lblContenidoMensajePago").html(
						"No puede continuar. " + data.error);
...
function procesarReporte() {
	window
			.open('../PymeCotizacionController?tipoConsulta=generarReporte&cotizacionId='
					+ $("#cotizacion_id").text());
}
...
function imprimirPoliza() {
	window
			.open('../PymeCotizacionController?tipoConsulta=imprimirPoliza&cotizacionId='
					+ $("#cotizacion_id").text());
}

function imprimirPolizaBorrador() {
	window
			.open('../PymeCotizacionController?tipoConsulta=imprimirPolizaBorrador&cotizacionId='
					+ $("#cotizacion_id").text());
}
...
function enviarPoliza() {
	...
	$.ajax({
		...
		success : function(data) {
			if (data.success) {
				$("#lbl_ErrorEmisionPoliza").html(
						"Póliza enviada correctamente");
				$("#msgPopupEmisionConError").show();
			} else {
				$("#lbl_ErrorEmisionPoliza")
						.html("No se pudo enviar la póliza");
				$("#msgPopupEmisionConError").show();
			}
...
function nuevoBeneficiario(endosoBeneficiarioId) {
	...
	$("#divEndosoBeneficiario")
			.append(
					'<table class="table table-bordered table-hover" id="fila_endoso_benficiario_'
							+ +contador
							+ '">'
							...
							+ 'id="eliminar_beneficiario_' + contador
							+ '" onClick="guardarBeneficiario(' + eliminar
							+ ',' + +contador + ');">Eliminar</button>'
							+ '</div></td></tr></table>');
	...
	$.ajax({
		...
		success : function(data) {
			...
			/*
			 * $('#beneficiario').select2({ data : beneficiarios, placeholder :
			 * 'Seleccione un beneficiario' });
			 */
			$("#beneficiario_" + contador).append(
					"<option value=''>Seleccione una opcion</option>");
			$.each(beneficiarios, function(index) {
				$("#beneficiario_" + contador).append(
						"<option value='" + beneficiarios[index].id + "'>"
								+ beneficiarios[index].text + "</option>");
			});
		}
	});
}
...
function validarValoresPagos(tipo) {
	...
	if (formulario != "1" || tipo != null) {
		...
		var valorCuotas = parseFloat(valorRestante).toFixed(2)
				/ parseInt(valorNumCuotas);
		...
		if (valorCuotaInicial == 0) {
			...
			if ($("#" + slctCts + " option[value='10']").length == 0)
				$(opcion).after(
						"<option value='10' class='" + clase
								+ "'>10 meses</option>");
			if (valorCuotas < 50) {
				valorNumCuotas = Math.floor(valorRestante / 50) == 0 ? 1 : Math
						.floor(valorRestante / 50);
				valorCuotas = parseFloat(valorRestante).toFixed(2)
						/ parseInt(valorNumCuotas);
				$(numCuotas).val(valorNumCuotas); // .trigger("change");
				alert("Las cuotas mensuales no pueden ser menores a $50. Usted puede pagar la diferencia en m&aacute;ximo "
						+ valorNumCuotas + " cuotas");
			}
		}
...
function validarCuotaMinima(event) {
	...
	var cuotaInicial = $(
			"#txtcuotaInicial" + aux.replace("txtcuotaInicial", "")).val();
	...
	if (valorCuotas < 50) {
		numCuotas = Math.floor(valorRestante / 50) == 0 ? 1 : Math
				.floor(valorRestante / 50);
		valorCuotas = parseFloat(valorRestante).toFixed(2)
				/ parseInt(numCuotas);
		$(target).val(numCuotas); // .trigger("change");
		alert("Las cuotas mensuales no pueden ser menores a $50. Usted puede pagar la diferencia en maximo "
				+ numCuotas + " cuotas");
		$("#" + aux + " option[value='10']").remove();
		var opcion = $("#" + id + " option[value='9']");
		var clase = $(opcion).attr("class");
		$(opcion).after(
				"<option value='10' class='" + clase + "'>10 meses</option>");
		calcularValoresCuotas();
	} else
		$(target).parent().parent().prev().children().last().children().first()
				.trigger("change");
}
...
function validarCuotaInicial(event) {
	...
	if ($(target).val() < 50 && $(target).val() != 0) {
		...
		$(target).parent().parent().next().children().last().children().first()
				.trigger("change");
		calcularValoresCuotas();
...
function calcularValoresCuotas() {
	...
	var cuotaInicial = "";
	var numeroCuotas = "";
	var valorTotal = $("#total").val().replace("$", "");
	var diferencia;
	var valorCuotas;
	...
	if (formulario != "1") {
		...
		var mensaje = "El valor total a pagar es $"
		...
		if (numeroCuotas != 0)
			$("#msgPopupPago").attr("class", "alert alert-info").html(mensaje)
					.fadeIn();
	}
}
					...
					if (data.success) {
						...
						if (datosCotizacion.etapa1 != null) {
							...
							if ($("#identificacion_asegurado").val() == "") {
								cargarTiposIdentificacionPymes(
										etapa1.tipoIdentificacion, 'asegurado',
										true);
								$("#identificacion_asegurado").val(
										etapa1.identificacion);
								$("#nombres_asegurado").val(etapa1.nombres);
								$("#nombre_completo_asegurado").val(
										etapa1.nombreCompleto);
						...
						if (datosCotizacion.etapa2 != null) {
							...
							$
									.each(
											...
											function(index) {
												if (index == 0) {
													$("#ubicacion_provincia_1")
															.val(
																	listadoDirecciones[index].provinciaId);
													obtenerCantonPorProvincia(
															listadoDirecciones[index].cantonId,
															"1");
													$("#cotizacionDetalleId_1")
															.val(
																	listadoDirecciones[index].cotizacionDetalleId);
													$(
															"#ubicacion_calle_principal_1")
															.val(
																	listadoDirecciones[index].callePrincipal);
													$("#ubicacion_numero_1")
															.val(
																	listadoDirecciones[index].numeroDireccion);
													$(
															"#ubicacion_calle_secundaria_1")
															.val(
																	listadoDirecciones[index].calleSecundaria);
													$("#contadorDirecciones")
															.val(
																	listadoDirecciones[index].cotizacionDetalleId);
												} else {
																					...
																					.ajax({
																						...
																						success : function(
																							...
																							if (data.success) {
																								tableR
																										.remove();
																								var numeroDirecciones = parseInt($(
																										"#contadorDirecciones")
																										.val())
																										- parseInt(1);
																								$(
													...
													obtenerCantonPorProvincia(
															...
															listadoDirecciones[index].cotizacionDetalleId);
													$(
															"#cotizacionDetalleId_"
																	+ listadoDirecciones[index].cotizacionDetalleId)
															.val(
																	listadoDirecciones[index].cotizacionDetalleId);
													$(
															"#ubicacion_calle_principal_"
																	+ listadoDirecciones[index].cotizacionDetalleId)
															.val(
																	listadoDirecciones[index].callePrincipal);
													$(
							...
							$
									.each(
											coberturasConfiguradas,
											function(index) {
												if (coberturasConfiguradas[index].tipoOrigen == "ADICIONALES") {
													cargarCobertura(
															coberturasConfiguradas[index].configuracionCoberturaId,
															"adicionales",
													...
													$(
															"#codigoGenerales"
																	+ coberturasConfiguradas[index].configuracionCoberturaId)
															.val(
																	coberturasConfiguradas[index].planId);
													cargarCoberturaGeneral(
															coberturasConfiguradas[index].configuracionCoberturaId,
															"general",
															coberturasConfiguradas[index].tasaSugerida,
															coberturasConfiguradas[index].tasaIngresada,
													...
													cargarDeducibleGeneral(
															coberturasConfiguradas[index].configuracionCoberturaId,
															coberturasConfiguradas[index].textoDeducible,
															coberturasConfiguradas[index].idsDeducible,
															coberturasConfiguradas[index].valoresDeducible,
															coberturasConfiguradas[index].textosDeducible);
												}
											});
						...
						if (datosCotizacion.etapa3 != null) {
							...
							if (etapa3.valoresCalculados != null) {
								$("#inspeccion_prima_sin_impuestos")
										.val(
												formatDollar(etapa3.valoresCalculados.valorPrima));
								$("#inspeccion_super_bancos")
										.val(
												formatDollar(etapa3.valoresCalculados.valorSuperBancos));
								$("#inspeccion_seguro_campesino")
										.val(
												formatDollar(etapa3.valoresCalculados.valorSeguroCampesino));
								$("#inspeccion_derecho_emision")
										.val(
												formatDollar(etapa3.valoresCalculados.valorDerechosEmision));
								$("#inspeccion_subtotal")
										.val(
												formatDollar(etapa3.valoresCalculados.valorSubTotal));
								$("#inspeccion_iva")
										.val(
												formatDollar(etapa3.valoresCalculados.valorIva));
								$("#inspeccion_total")
										.val(
												formatDollar(etapa3.valoresCalculados.valorTotal));
								...
								$("#resumenTotalPago")
										...
										.append(
												"$"
														+ etapa3.valoresCalculados.valorTotal);
							}
						...
						if (datosCotizacion.etapa4 != null) {
							...
							if (etapa4.endosoBeneficiario != null) {
								if (etapa4.endosoBeneficiario.endososBeneficiario != null
										|| etapa4.endosoBeneficiario.endososBeneficiario != "") {
								...
								$("#asegurado_id").val(
										etapa4.endosoBeneficiario.entidadId);
								$("#identificacion_asegurado")
										.val(
												etapa4.endosoBeneficiario.identificacion);
								$("#nombres_asegurado").val(
										etapa4.endosoBeneficiario.nombres);
								$("#apellidos_asegurado").val(
										etapa4.endosoBeneficiario.apellidos);
								$("#nombre_completo_asegurado")
										.val(
												etapa4.endosoBeneficiario.nombreCompleto);

								cargarTiposIdentificacionPymes(
										etapa4.endosoBeneficiario.tipoIdentificacion,
										'asegurado', true);
								$("#endoso_beneficiario_id")
										.val(
												etapa4.endosoBeneficiario.endosoBeneficiarioId);
							} else {
								cargarPestanaEndosoAsegurado(datosCotizacion.etapa1.identificacion);
							}

							if (etapa4.formaPago != null
									&& etapa4.formaPago.pagoId != null) {
								...
								cargarFormasPago(etapa4.formaPago.formaPagoId,
										"formasPago");

								if (etapa4.formaPago.formaPagoNombre.trim()
										.toString() == "DEBITO BANCARIO") {
									cargarTiposIdentificacionPymes("",
											"tarjeta", false);
									cargarTiposIdentificacionPymes(
											etapa4.formaPago.tipoIdentificacion,
											"banco", true);
									$("#bancoNumeroCuenta").val(
											etapa4.formaPago.numCuentaTarjeta);
									$("#bancoTitular").val(
											etapa4.formaPago.nombreTitular);
									$("#bancoIdentificacion")
											.val(
													etapa4.formaPago.identificacionTitular);
									$("#txtcuotaInicialbancoPlazo").val(
											etapa4.formaPago.cuotaInicial);
									cargarFormasPago(
											etapa4.formaPago.institucionFinancieraId,
											"intitucionesFinancieras");
									$("#bancoTipoCuenta").val(
											etapa4.formaPago.tipoCuenta);
									// $("#bancoTipoIdentificacion
									// option[value='"+
									// etapa3.formaPago.tipoIdentificacion
									// +"']").attr("selected", "selected");
									$("#txtfechaInicialpago").val(
											etapa4.formaPago.fechaDebito);
									$("#bancoPlazo")
											.val(etapa4.formaPago.plazo);
									if (etapa4.formaPago.cuotaInicial > 0)
										$("#bancoPlazo option[value='10']")
												.remove();
								}
								...
								if (etapa4.formaPago.formaPagoNombre.trim()
									...
									cargarFormasPago(
											...
											"intitucionesFinancieras");
									cargarFormasPago(
											etapa4.formaPago.anioExpTarjeta,
											"aniosVigencia");
									$("#tarjetaTipoCuenta").val(
											etapa4.formaPago.tipoCuenta);
									$("#tarjetaNumero").val(
											etapa4.formaPago.numCuentaTarjeta);
									$("#tarjetaTitular").val(
											etapa4.formaPago.nombreTitular);
									$("#tarjetaMesExp").val(
											etapa4.formaPago.mesExpTarjeta);
									...
									$("#tarjetaIdentificacion")
											.val(
													etapa4.formaPago.identificacionTitular);
									$("#tarjetaPlazo").val(
											etapa4.formaPago.plazo);
									$("#txtfechaInicialpagoTarjeta").val(
											etapa4.formaPago.fechaDebito);
									if (etapa4.formaPago.cuotaInicial > 0)
										$("#tarjetaPlazo option[value='10']")
												.remove();
								}
						...
						if (datosCotizacion.estadoCotizacion == "Borrador") {
							...
							$("#imprimirAutorizacionDebito").show();
							$("#imprimirAutorizacionDebito").removeAttr(
									"disabled");
							$("#imprimirVinculacionCliente").show();
							$("#imprimirVinculacionCliente").removeAttr(
									"disabled");
							$("a[href='#next']").click();
							$("a[href='#next']").click();
						}
...
function cargarResumenValores() {
	$
			.ajax({
				...
				datatype : 'json',
				success : function(data) {
					$("#inspeccion_total_suma_asegurada").val(
							formatDollar(data.valorAsegurado));
					$("#inspeccion_prima_sin_impuestos").val(
							formatDollar(data.valorPrima));
					$("#inspeccion_super_bancos").val(
							formatDollar(data.valorSuperBancos));
					if (parseFloat(data.porcentajeDescuento) > 0)
						$("#inspeccion_filaDescuento").show();
					$("#inspeccion_porcentaje_descuento").val(
							formatDollar(data.porcentajeDescuento));
					$("#inspeccion_seguro_campesino").val(
							formatDollar(data.valorSeguroCampesino));
					$("#inspeccion_derecho_emision").val(
							formatDollar(data.valorDerechosEmision));
					$("#inspeccion_subtotal").val(
							formatDollar(data.valorSubTotal));
					...
					$("#total_suma_asegurada").val(
							formatDollar(data.valorAsegurado));
					$("#prima_sin_impuestos")
							.val(formatDollar(data.valorPrima));
					$("#super_bancos").val(formatDollar(data.valorSuperBancos));
					if (parseFloat(data.porcentajeDescuento) > 0)
						$("#filaDescuento").show();
					$("#porcentaje_descuento").val(
							formatDollar(data.porcentajeDescuento));
					$("#seguro_campesino").val(
							formatDollar(data.valorSeguroCampesino));
					$("#derecho_emision").val(
							formatDollar(data.valorDerechosEmision));
							...
							if (entidad === undefined) {
								...
								if (entidad.clienteIdEnsurance != ""
									...
									if (formulario == "datosClienteInicio") {
										...
										$("#tipo_identificacion_principal")
												.val(entidad.tipoIdentificacion)
												.trigger("change");
									}
						...
						if (configCobertasPorProducto[index].tipoDeclaracion == "1") {
							if ($(
									...
									&& ($(
											"#planId_"
													+ configCobertasPorProducto[index].configuracionCoberturaId)
											.val() == configCobertasPorProducto[index].planId)) {
								var tasaSugerida = 0;
								var tasaIngresada = 0;
								var montoAsegurado = 0;
								var primaCalculada = 0;
								var planGeneral = 0;
								if ($("#tasa_sugerida_"
										+ configCobertasPorProducto[index].configuracionCoberturaId).length)
									tasaSugerida = Number($(
											"#tasa_sugerida_"
													+ configCobertasPorProducto[index].configuracionCoberturaId)
											.val());
								if ($("#planes_general_cobertura_"
										+ configCobertasPorProducto[index].configuracionCoberturaId).length)
									planGeneral = $(
											"#planes_general_cobertura_"
													+ configCobertasPorProducto[index].configuracionCoberturaId)
											.val();
								if ($("#tasa_ingresada_"
										+ configCobertasPorProducto[index].configuracionCoberturaId).length)
								...
								var cobertura = {
									...
									valorIngresado : montoAsegurado,
									primaCalculada : primaCalculada,
									tipoOrigen : configCobertasPorProducto[index].tipoCoberturaId == 1 ? "GENERAL"
											: "ADICIONALES",
									textoDeducible : "",
									idsDeducible : "",
									valoresDeducible : "",
			...
			.each(
					...
					function(index) {
						if ($(
							...
							if ($("#valor_asistencia_"
									+ configAsistenciasPorProducto[index].asistenciaId).length)
				...
				.ajax({
					...
					success : function(data) {
						if (data.success) {
							...
							if (tipoConsulta == "eliminar") {
								$("#fila_endoso_benficiario_" + itemId)
										.remove();
								$("#mensajeGraboEndosoBeneficiario")
										.html(
...
function abrirCertificadoDebito() {
	window
			.open('../PymeCotizacionController?tipoConsulta=imprimirAutorizacionDebito&cotizacionId='
					+ $("#cotizacion_id").text());
}

function abrirVinculacionCliente() {
	window
			.open('../PymeCotizacionController?tipoConsulta=imprimirVinculacionCliente&cotizacionId='
					+ $("#cotizacion_id").text());
}

/* pfalconi 2/17/2016 */
function obtenerTipoPredioPorActividad(seleccionada) {
	tipoPredioList.value([]);
	// Consultar la ACTIVIDAD
	$.ajax({
		url : '../TipoPredioController',
		data : {
			"tipoConsulta" : "encontrarPorActividad",
			"actividadEconomicaId" : $("#actividad_economica option:selected")
					.val()
		},
		async : false,
		type : 'post',
		datatype : 'json',
		success : function(data) {
			tipoPredioList.dataSource.filter({});
			tipoPredioList.setDataSource(data.tiposPredio);
			$("#giro_negocio").data("kendoMultiSelect").value(seleccionada);
		}
	});
}

function cargarPlanEstructuras() {
	$("#planes_estructuras").empty();

	// Consultar la planes estructuras
	$.ajax({
		url : '../PymeConfiguracionCoberturaController',
		data : {
			"tipoConsulta" : "encontrarPorPlanesEstructuras",
			"grupoPorProductoId" : $("#ProductoID").val(),
			"tipoDeclaracionId" : "3",
		},
		async : false,
		type : 'post',
		datatype : 'json',
		success : function(data) {
			var listadoTipos = data.listaPlanesEstructuras;
			$("#planes_estructuras").append(
					"<option value='-1'>Seleccione un Plan</option>");
			$.each(listadoTipos, function(index) {
				$("#planes_estructuras")
						.append(
								"<option value='" + listadoTipos[index].codigo
										+ "'>" + listadoTipos[index].nombrePlan
										+ " - "
										+ listadoTipos[index].valormaximo
										+ "</option>");
...
// Consultar la planes Contenidos
function cargarPlanContenidos() {
	$("#planes_contenidos").empty();
	$.ajax({
		url : '../PymeConfiguracionCoberturaController',
		data : {
			"tipoConsulta" : "encontrarPorPlanesContenidos",
			"grupoPorProductoId" : $("#ProductoID").val(),
			"tipoDeclaracionId" : "4",
		},
		async : false,
		...
		datatype : 'json',
		success : function(data) {
			var listadoTipos = data.listaPlanesContenidos;
			$("#planes_contenidos").append(
					"<option value='-1'>Seleccione un Plan</option>");
			$.each(listadoTipos, function(index) {
				$("#planes_contenidos")
						.append(
...
function cargarCamposOcultos() {
	...
	if (valor2 == undefined)
		valor2 = 'SIN PLAN';
	var resEstructuras = valor2.substring(0, 7).trim();
	var valor1 = $("#planes_estructuras option:selected").html();
	if (valor1 == undefined)
		valor1 = 'SIN PLAN';
	var res = valor1.substring(8, 15);
	var valor2 = $("#planes_estructuras").val();
	if ("-1" != valor2) {
		$("#valor_estructura").val(res.trim());
		$("#tituloEstructuras").text("Estructura " + resEstructuras);
	} else {
		$("#valor_estructura").val("");
		$("#tituloEstructuras").text("");
	}
	var valor = $("#planes_contenidos option:selected").html();
	if (valor == undefined)
		valor = 'SIN PLAN';
	var res1 = valor.substring(8, 15);
	var valor3 = $("#planes_contenidos").val();
	if ("-1" != valor3) {
		$("#valor_contenidos").val(res1.trim());
		$("#tituloContenidos").text("Contenido " + resContenidos);
	} else {
		$("#valor_contenidos").val("");
		$("#tituloContenidos").text("");
	}
}

function cargarPlanGenerales(coberturaId) {
	$("#planes_general_cobertura_" + coberturaId).empty();
	// Consultar la planes generales
	$.ajax({
		url : '../PymeConfiguracionCoberturaController',
		data : {
			"tipoConsulta" : "encontrarPorPlanesGenerales",
			"grupoPorProductoId" : $("#ProductoID").val(),
			"configuracionCoberturaId" : coberturaId,
			"tipoDeclaracionId" : "1",
		},
		async : false,
		type : 'post',
		datatype : 'json',
		success : function(data) {
			var listadoTipos = data.listaPlanesGenerales;
			$("#planes_general_cobertura_" + coberturaId).append(
					"<option value='-1'>Seleccione un Plan</option>");
			$.each(listadoTipos, function(index) {
				$("#planes_general_cobertura_" + coberturaId)
						.append(
								"<option value='" + listadoTipos[index].codigo
										+ "'>" + listadoTipos[index].nombrePlan
										+ " - "
...
function calcularTotalValorAseguradoPlanGenerales(coberturaId) {
	$("#detalle_cobertura_general_" + coberturaId).empty();
	var valor1 = $("#codigoGenerales" + coberturaId).val();
	$("#cobertura_" + coberturaId).prop("checked", true);
	$("#detalle_cobertura_general" + coberturaId).append(
			crearCoberturaGeneral(coberturaId, "general", valor1));
}

function cargarDeducibles(configuracionCoberturaId, tipoVariable,
		coberturaNombre) {
	$
			.ajax({
				url : '../PymeConfiguracionCoberturaController',
												...
												$("#deducibles_por_direccion")
														.append(
																'<tr id="deducible_cobertura_'
																		...
																		+ configuracionCoberturaId
																		+ '" value="'
																		+ listadoTipos[index].idsDeducible
																		+ '"><input type="hidden" id="valores_deducible_cobertura_'
																		+ configuracionCoberturaId
																		+ '" value="'
																		+ listadoTipos[index].valoresDeducible
																		+ '"><input type="hidden" id="textos_deducible_cobertura_'
																		+ configuracionCoberturaId
																		+ '" value="'
																		+ listadoTipos[index].textosDeducible
																		+ '"></td></tr></table></td></tr>');
											}
										}
									});
...
function cargarPlanEstructurasGroup() {

	$("#selectEstructura").empty();
	// Consultar la planes Contenidos
	$
			.ajax({
				url : '../PymeConfiguracionCoberturaController',
				...
				success : function(data) {
					...
					$("#selectEstructura")
							.append(
									"<button  name='estru' class='btn  btn-primary btne datosPymes' value='-1'>NINGUN<br/>PLAN</button>");
					$.each(listadoTipos, function(index) {
						$("#selectEstructura").append(
								"<button  name='estru' class='btn  btn-primary btne datosPymes' value='"
										+ listadoTipos[index].codigo + "'>"
										+ listadoTipos[index].nombrePlan
										+ "<br/> $"
										+ listadoTipos[index].valormaximo
										+ "</button>");
...
function calcularTotalValorAseguradoPlanEstructuras(codigo, texto, rol) {
	var valor1 = texto;
	var res = valor1.substring(8, 15);
	var resEstructuras2 = valor1.substring(0, 7).trim();
	var valor2 = codigo;
	if ("-1" != valor2) {
		$("#valor_estructura").val(res.trim());
		$("#tituloEstructuras").text("Estructura " + resEstructuras2);
	} else {
		$("#valor_estructura").val("");
		$("#tituloEstructuras").text("");
		$("#coberturas_por_direccionEstructuras").empty();
		$("#prima_neta_por_direccion").text(formatDollar(0.0));
		var filterCoberturasDependen = $(configCobertasPorProducto)
				.filter(
						function(idx) {
							return ((configCobertasPorProducto[idx].tipoDeclaracion == "3" || configCobertasPorProducto[idx].dependeValor == "2"));
						});
		$
				.each(
						filterCoberturasDependen,
	...
	var numerictextbox = $("#total_valor_asegurado")
			.data("kendoNumericTextBox");
	numerictextbox.value("0.0");
	var valEst = Number($("#valor_estructura").val());
	var valMue = Number($("#valor_contenidos").val());

	var valor1 = codigo;
	var valor2 = $("#valor_estructura").val();
	if ("-1" == valor1) {
		$("#coberturas_por_direccionEstructuras").empty();
		// Calculo total de la prima de esta direccion
		var valorTotal = 0;
	...
	var filterCalculaValorAsegurado = $(configCobertasPorProducto)
			.filter(
					function(idx) {
						return (configCobertasPorProducto[idx].dependeValor == "2" && configCobertasPorProducto[idx].planId == codigo);
					});
	/*
	 * var acumulador = Number("0.0"); $.each(filterCalculaValorAsegurado,
	 * function(index) { acumulador =
	 * acumulador+filterCalculaValorAsegurado[index].valorAsegurado; });
	 * $("#valor_estructura").val(valEst+acumulador);
	 */
	$("#valor_estructura").val(valEst);
	var numerictextbox = $("#total_valor_asegurado")
			.data("kendoNumericTextBox");
	numerictextbox.value(valEst + valMue);

	// var tabStrip = $("#tabstrip").kendoTabStrip().data("kendoTabStrip");
	$("#advertencia_general").html("");
	// if(rol=='ADM. COMERCIAL VH, PYMES')
	// tabStrip.enable(tabStrip.tabGroup.children().eq(2), true);
	// else
	// tabStrip.enable(tabStrip.tabGroup.children().eq(2), false);
	...
	$("#coberturas_por_direccionEstructuras").append('<tr><th>Cobertura</th><th>Ramo</th></tr>');
	// Busco todas las coberturas que dependen de un valor
	var filterCoberturasDependen = null;
	var filterCoberturasDependen = $(configCobertasPorProducto)
			.filter(
					function(idx) {
						return ((configCobertasPorProducto[idx].tipoDeclaracion == "3" || configCobertasPorProducto[idx].dependeValor == "2") && configCobertasPorProducto[idx].planId == valor1);
					});
	...
	$
			.each(
					filterCoberturasDependen,
					function(index) {
						$("#coberturas_por_direccionEstructuras")
								.append(

										'<tr><td style="color: green;"><input type="checkbox"  disabled="" id="cobertura_'
												+ filterCoberturasDependen[index].configuracionCoberturaId
												+ '" class="datosPymes coberturaPorDireccion" onclick=crearCobertura('
												+ filterCoberturasDependen[index].configuracionCoberturaId
												+ ',"direccion"'
												+ ','
												+ filterCoberturasDependen[index].planId
												+ ')>'
												+ filterCoberturasDependen[index].coberturaNombre
												+ '</td><td style="color: green;">('+filterCoberturasDependen[index].ramoNombre+')</td></tr>');
						$(
								"#cobertura_"
										+ filterCoberturasDependen[index].configuracionCoberturaId)
								.prop("checked", true);
						crearCobertura(
								filterCoberturasDependen[index].configuracionCoberturaId,
								"direccionEstructuras",
								filterCoberturasDependen[index].planId);
					});

}

function calcularTotalValorAseguradoPlanContenidos(codigo, texto, rol) {
	var valor = texto;
	var res1 = valor.substring(8, 15);
	var resContenidos2 = valor.substring(0, 7).trim();
	var valor3 = codigo;
	if ("-1" != valor3) {
		$("#valor_contenidos").val(res1.trim());
		$("#tituloContenidos").text("Contenido " + resContenidos2);
	} else {
		$("#valor_contenidos").val("");
		$("#tituloContenidos").text("");
		var filterCoberturasDependen = $(configCobertasPorProducto)
				.filter(
						function(idx) {
							return ((configCobertasPorProducto[idx].tipoDeclaracion == "4" || configCobertasPorProducto[idx].dependeValor == "8"));
						});
	...
	var valMue = Number($("#valor_contenidos").val());
	var valor1 = codigo;
	var valor2 = $("#valor_contenidos").val();
	if ("-1" == valor1) {
		$("#coberturas_por_direccionContenidos").empty();
		// Calculo total de la prima de esta direccion
		var valorTotal = 0;
	...
	// Busco todas las coberturas que dependen de un valor
	$("#coberturas_por_direccionContenidos").append('<tr><th>Cobertura</th><th>Ramo</th></tr>');
	var filterCoberturasDependen = null;
	var filterCoberturasDependen = $(configCobertasPorProducto)
			.filter(
					function(idx) {
						return ((configCobertasPorProducto[idx].tipoDeclaracion == "4" || configCobertasPorProducto[idx].dependeValor == "8") && configCobertasPorProducto[idx].planId == codigo);
					});
	...
	$
			.each(
					filterCoberturasDependen,
					function(index) {
						$("#coberturas_por_direccionContenidos")
								.append(
										'<tr><td style="color: green;"><input type="checkbox" disabled="" id="cobertura_'
												+ filterCoberturasDependen[index].configuracionCoberturaId
												+ '" class="datosPymes coberturaPorDireccion" onclick=crearCobertura('
	...
	// recalcula los valores de estructuras
	calcularTotalValorAseguradoPlanEstructuras($('.Estructura > .btne.active')
			.val(), $('.Estructura > .btne.active').text(), rol);
}

function cargarPlanContenidosGroup() {
	...
	$
			.ajax({
				...
				success : function(data) {
					...
					$("#selectContenido")
							.append(
									"<button  name='cont' class='btn  btn-primary btnc datosPymes' value='-1'>NINGUN<br/>PLAN</button>");
					$.each(listadoTipos, function(index) {
						$("#selectContenido").append(
								"<button  name='cont' class='btn btn-primary btnc datosPymes' value='"
										+ listadoTipos[index].codigo + "'>"
										+ listadoTipos[index].nombrePlan
										+ "<br/> $"
										+ listadoTipos[index].valormaximo
...
function cargarPlanGeneralesGroup(coberturaId, dependeValor) {
	$("#planes_general_cobertura_" + coberturaId).empty();
	// Consultar la planes generales
	$
			.ajax({
				...
				success : function(data) {
					...
					$("#planes_general_cobertura_" + coberturaId)
							.append(
									"<button style='font-size: 80%; font-weight:bold' type='button' class='btn btn-success btn-md btng"
											+ coberturaId
											+ " datosPymes' value='-1'>NINGUN<br/>PLAN</button>");
					$
							.each(
									listadoTipos,
									function(index) {
...
function determinarTasa(configuracionCoberturaId, valorAsegurado) {
	var filterResult = $(configCobertasPorProducto).filter(function(idx) {
		return configCobertasPorProducto[idx].configuracionCoberturaId === configuracionCoberturaId;
	});
	var tasa;
	if(filterResult[0].tipoDeclaracionNombre=="GENERAL"){
		...
		if(contadorDirecciones!="0"){
			...
			if(tasa==0)
				tasa=determinarTasaNormal(configuracionCoberturaId, valorAsegurado);
			}else{
				tasa=determinarTasaNormal(configuracionCoberturaId, valorAsegurado);
			}
		}else{
...
function determinarTasaNormal(configuracionCoberturaId, valorAsegurado) {
	var tasa = 0;
	var filterResult = $(configCobertasPorProducto)
			.filter(
					function(idx) {
						...
						success : function(data) {
							...
							$
									.each(
											listadoTasas,
											function(index) {
												if (Number(valorAsegurado) >= Number(listadoTasas[index].valorCoberturaInicial)
														&& Number(valorAsegurado) <= Number(listadoTasas[index].valorCoberturaFinal)) {
													tasa = Number(listadoTasas[index].tasa);
												}
											});
						}
					...
					.ajax({
						...
						success : function(data) {
							if (data.tasa) {
								...
								var filterResult2 = $(configCobertasPorProducto)
										.filter(
												function(idx) {
													return configCobertasPorProducto[idx].coberturaId === filterResult[0].coberturaCopiaId;
												});
								if (filterResult2.length > 0) {
									tasa = Number($(
											"#tasa_ingresada_"
													+ filterResult2[0].configuracionCoberturaId)
											.val());
								}
							}
						}
...
function crearCoberturaGeneral(configuracionCoberturaId, tipo, plan) {
	...
	if (filterResult1.length > 0) {
		if ($("#cobertura_" + configuracionCoberturaId).is(':checked')) {
			var valorMaximo = filterResult1[0].valorAsegurado;
			var tasa = determinarTasa(configuracionCoberturaId,
					filterResult1[0].valorAsegurado);
			if (filterResult1[0].dependeValor == "10") {
				tasa = 0;
				valorMaximo = determinarMaximoGeneral();
			}
			var clase = filterResult1[0].incluyeEnProducto == "1" ? "predeterminado"
					: "normal";
			var formacionFila = '<tr class="' + clase
			...
			formacionFila = formacionFila
					...
					+ configuracionCoberturaId
					+ '" class="datosPymes porcentajeTasasPymes" onchange="calcularPrimaNeta('
					+ configuracionCoberturaId + ',' + valorMaximo + ',' + plan
					+ ')"></td>';
			if (filterResult1[0].dependeValor == "10") {
				formacionFila = formacionFila
						+ '<td>Monto Asegurado:</td>'
						+ '<td><input type="text" style="width: 100px" id="monto_asegurado_'
						+ configuracionCoberturaId;
				if (tipo == "direccionContenidos"
						|| tipo == "direccionEstructuras")
					formacionFila = formacionFila
							+ '" class="datosPymes datosMontoAseguradoDir" onchange="calcularPrimaNeta(';
				else
					formacionFila = formacionFila
				...
				formacionFila = formacionFila
						+ '<td><input type="hidden" style="width: 100px" id="monto_asegurado_'
						+ configuracionCoberturaId;
				if (tipo == "direccionContenidos"
						|| tipo == "direccionEstructuras")
					formacionFila = formacionFila
			...
			cargarDeduciblesGenerales(configuracionCoberturaId,
					filterResult1[0].coberturaNombre);

			$("#tasa_sugerida_" + configuracionCoberturaId).val(tasa);
			$("#tasa_ingresada_" + configuracionCoberturaId).val(tasa);
			if (filterResult1[0].dependeValor == "10") {
				$("#monto_asegurado_" + configuracionCoberturaId).val(0);
			} else {
				$("#monto_asegurado_" + configuracionCoberturaId).val(
						valorMaximo);
				calcularPrimaNeta(configuracionCoberturaId, valorMaximo, plan);
...
function cargarDeduciblesGenerales(configuracionCoberturaId, coberturaNombre) {
	$
			.ajax({
				url : '../PymeConfiguracionCoberturaController',
				data : {
