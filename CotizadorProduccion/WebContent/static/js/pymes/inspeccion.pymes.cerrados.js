...
function obtenerConfiguracionDireccion(numeroDireccion) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			var valorPisos = data.numeroTotalPisos;
			if(valorPisos == '0')
				valorPisos = '';
			numerictextbox1.value(valorPisos);
...
function cargarPorCotizacionId(){
	$.ajax({
				url : '../PymesObjetoCotizacionController',
		data : {
			"tipoConsulta" : "obtenerDireccionesSinInspeccion",
			"cotizacionId" : $("#cotizacion_id").text()
		},
		async : false,
		type : 'post',
...
//	}
//	if($("#txt_Latitud").val()==""){
//		alert("Debe ingresar la latitud para poder grabar la inspección");
//		return;
//	}
//	if($("#txt_Longitud").val()==""){
