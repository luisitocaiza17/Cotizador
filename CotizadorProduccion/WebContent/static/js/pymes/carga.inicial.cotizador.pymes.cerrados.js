...
function cargar(){
	...
	var today = new Date();
	//riesgo
	$("#riesgoDanio_si").attr('checked', 'checked');
	$('#tieneRiesgo').val(0);
...
function cargaInicialPymes(){
	...
	$("#registrarParaInspeccion").click(function (e) {
		...
		var id=$("#cotizacion_id").text();
		window.location.href='InspeccionPymesCerrados.jsp?id='+id;	
	});
	...
	tipoPredioList = $("#giro_negocio").data("kendoMultiSelect");
}
...
function cargarTiposPredios(){
	$.ajax({
		...
		datatype : 'json',
		success : function (data) {
			var listadoGrupos = data.tiposPredio;	
						
			$("#giro_negocio").append("<option value=''>Seleccione una opcion</option>");
			$.each(listadoGrupos, function (index) {
				$("#giro_negocio").append("<option value='" + listadoGrupos[index].codigo + "'>" + listadoGrupos[index].nombre + "</option>");
			});
		}
	});
}
